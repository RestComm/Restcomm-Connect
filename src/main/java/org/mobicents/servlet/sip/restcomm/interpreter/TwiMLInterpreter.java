...
public final class TwiMLInterpreter extends FSM implements Runnable, TagVisitor {
  // Logger.
  private static final Logger logger = Logger.getLogger(TwiMLInterpreter.class);
  // Interpreter states.
  ...
  private final TwiMLInterpreterContext context;
  ...
  public TwiMLInterpreter(final TwiMLInterpreterContext context) {
    super(IDLE);
    addState(IDLE);
    addState(REDIRECTED);
    addState(READY);
    ...
    this.factory = new TwiMLTagStrategyFactory();
  }
  ...
  public void initialize() throws InterpreterException {
    try {
      ...
      setState(READY);
    } catch(final TwiMLInterpreterContextException exception) {
      setState(FAILED);
  ...
  public void run() {
    while(getState().equals(READY)) {
      ...
      while(iterator.hasNext()) {
        final TwiMLTag tag = (TwiMLTag)iterator.next();
