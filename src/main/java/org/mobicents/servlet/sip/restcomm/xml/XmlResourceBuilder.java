...
public final class XmlResourceBuilder {
  ...
  public XmlResourceBuilder(final TagFactory factory) {
    super();
    this.factory = factory;
  }
  
  public XmlResource build(final InputStream input) throws XmlResourceBuilderException {
    try {
  	  ...
  	  switch(xmlStreamReader.next()) {
  	    ...
  	    case XMLStreamConstants.END_DOCUMENT: {
  	      if(!tagStack.isEmpty() && tagStack.size() == 1) {
  	    	tag = tagStack.pop();
  	    	if(!tag.canBeRoot()) {
  	    	  throw new XmlResourceBuilderException("The tag <" + tag.getName() + "> can not be a RestComm XML resource root tag.");
  	    	}
  	      }
  	      continue;
  	    }
