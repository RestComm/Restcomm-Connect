...
import org.mobicents.servlet.restcomm.sms.GetLastSmsEvent;
import org.mobicents.servlet.restcomm.sms.GetSmsSessionInfo;
import org.mobicents.servlet.restcomm.sms.SendSms;
import org.mobicents.servlet.restcomm.sms.SmsEvent;
...
public final class SmsInterpreter extends UntypedActor {
  ...
  private final State acquiringSmsSessionInfo;
  private final State acquiringLastSmsEvent;
  ...
  // The information for this sms session.
  private SmsSessionInfo smsSessionInfo;
  ...
  public SmsInterpreter(final ActorRef service, final ActorRef downloader,
      final DaoManager storage, final Configuration configuration,
      final Sid accountId, final Sid phoneId, final String version,
      final URI url, final String method, final URI fallbackUrl,
      final String fallbackMethod) {
    super();
    final ActorRef source = self();
    uninitialized = new State("uninitialized", null, null);
    acquiringSmsSessionInfo = new State("acquiring sms session info",
        new AcquiringSmsSessionInfo(source), null);
    acquiringLastSmsEvent = new State("acquiring last sms event",
        new AcquiringLastSmsEvent(source), null);
    ...
    sendingSms = new State("sending sms", null,null);
    finished = new State("finished", null, null);
    // Initialize the transitions for the FSM.
    final Set<Transition> transitions = new HashSet<Transition>();
    transitions.add(new Transition(uninitialized, acquiringSmsSessionInfo));
    transitions.add(new Transition(acquiringSmsSessionInfo, acquiringLastSmsEvent));
    transitions.add(new Transition(acquiringSmsSessionInfo, finished));
    transitions.add(new Transition(acquiringLastSmsEvent, downloadingRcml));
    transitions.add(new Transition(acquiringLastSmsEvent, finished));
    ...
    transitions.add(new Transition(ready, sendingSms));
  ...
  @Override public void onReceive(final Object message) throws Exception {
    ...
    if(StartInterpreter.class.equals(klass)) {
      fsm.transition(message, acquiringSmsSessionInfo);
    } else if(SmsSessionInfo.class.equals(klass)) {
      fsm.transition(message, acquiringLastSmsEvent);
    } else if(SmsEvent.class.equals(klass)) {
  ...
  private abstract class AbstractAction implements Action {
    ...
    protected List<NameValuePair> parameters() {
      ...
      final String smsSessionSid = smsSessionInfo.sid().toString();
  	  ...
  	  final String from = format(smsSessionInfo.from());
      parameters.add(new BasicNameValuePair("From", from));
      final String to = format(smsSessionInfo.to());
      parameters.add(new BasicNameValuePair("To", to));
      final String body = smsEvent.body();
      parameters.add(new BasicNameValuePair("Boy", body));
      return parameters;
    }
  ...
  private final class AcquiringSmsSessionInfo extends AbstractAction {
    public AcquiringSmsSessionInfo(final ActorRef source) {
      super(source);
    }
	...
	@Override public void execute(final Object message) throws Exception {
      ...
      session.tell(new GetSmsSessionInfo(), source);
	}
  }
  ...
  private final class AcquiringLastSmsEvent extends AbstractAction {
	...
	@Override public void execute(final Object message) throws Exception {
      smsSessionInfo = (SmsSessionInfo)message;
      final GetLastSmsEvent request = new GetLastSmsEvent();
      session.tell(request, source);
	}
  }
  ...
  private final class DownloadingRcml extends AbstractAction {
	...
	@Override public void execute(final Object message) throws Exception {
      smsEvent = (SmsEvent)message;
      final SmsMessage.Builder builder = SmsMessage.builder();
      builder.setSid(smsSessionInfo.sid());
      builder.setAccountSid(accountId);
      builder.setApiVersion(version);
      builder.setRecipient(smsSessionInfo.to());
      builder.setSender(smsSessionInfo.from());
      builder.setBody(smsEvent.body());
      builder.setDirection(Direction.INCOMING);
      builder.setStatus(Status.RECEIVED);
      builder.setPrice(new BigDecimal("0.00"));
      final StringBuilder buffer = new StringBuilder();
	  buffer.append("/").append(version).append("/Accounts/");
	  buffer.append(accountId.toString()).append("/SMS/Messages/");
	  buffer.append(smsSessionInfo.sid().toString());
	  final URI uri = URI.create(buffer.toString());
	  builder.setUri(uri);
	  final SmsMessage sms = builder.build();
	  final SmsMessagesDao messages = storage.getSmsMessagesDao();
	  messages.addSmsMessage(sms);
  ...
  private final class Redirecting extends AbstractAction {
	...
	@Override public void execute(final Object message) throws Exception {
      final Tag redirect = (Tag)message;
      final NotificationsDao notifications = storage.getNotificationsDao();
      String method = redirect.attribute("method").value();
      ...
      final String text = redirect.text();
  ...
  private final class SendingSms extends AbstractAction {
	...
	@Override public void execute(final Object message) throws Exception {
      final Tag sms = (Tag)message;
      final NotificationsDao notifications = storage.getNotificationsDao();
      // Parse "from".
      String from = sms.attribute("from").value();
      if(from == null || from.isEmpty()) {
        from = smsSessionInfo.to();
      } else {
        from = format(from);
        if(from == null) {
          from = sms.attribute("from").value();
      ...
      // Parse "to".
      String to = sms.attribute("to").value();
      if(to == null || to.isEmpty()) {
        to = smsSessionInfo.from();
      } else {
        to = format(to);
        if(to == null) {
          to = sms.attribute("to").value();
      ...
      if(body == null || body.isEmpty() || body.length() > 160) {
        ...
        // Send the SMS.
        session.tell(new SendSms(from, to, body), source);
      }
      // Parses "action".
      String action = sms.attribute("action").value();
      ...
      // Parse "method".
      String method = sms.attribute("method").value();
      if(method != null && !method.isEmpty()) {
        if(!"GET".equalsIgnoreCase(method) && !"POST".equalsIgnoreCase(method)) {
      ...
      String callback = sms.attribute("statusCallback").value();
	}
  }
}
