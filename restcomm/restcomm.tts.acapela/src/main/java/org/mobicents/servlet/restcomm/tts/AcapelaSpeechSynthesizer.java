...
public final class AcapelaSpeechSynthesizer extends UntypedActor {
	...
	private URI synthesize(final Object message)
		...
		if(speaker == null) {
			...
			throw new IllegalArgumentException("There is no suitable speaker to synthesize " +
					request.language());
		}
		...
		if(status == HttpStatus.SC_OK) {
			...
			if("OK".equals(results.get("res"))) {
				...
				logger.info("AcapelaSpeechSynthesizer error code: "+results.get("err_code")+" error message: "+results.get("err_msg"));
			...
			logger.info("AcapelaSpeechSynthesizer error, status code: "+line.getStatusCode()+(" reason phrase: ")+line.getReasonPhrase());
			final StringBuilder buffer = new StringBuilder();
