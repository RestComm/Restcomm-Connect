...
public class VoxbonePhoneNumberProvisioningManager implements PhoneNumberProvisioningManager {
     ...
     * (non-Javadoc)
     *
     * @see PhoneNumberProvisioningManager#init(org.apache.commons.configuration .Configuration, boolean)
     */
    @Override
    public void init(Configuration phoneNumberProvisioningConfiguration, Configuration telestaxProxyConfiguration,
        ...
        if (clientResponse.getClientResponseStatus() == Status.OK) {
            ...
            clientResponse = webResource.queryParam(PAGE_NUMBER, "0").queryParam(PAGE_SIZE, "300").accept(CONTENT_TYPE)
                    .type(CONTENT_TYPE).get(ClientResponse.class);
    ...
    private List<PhoneNumber> toAvailablePhoneNumbers(final JsonArray phoneNumbers, PhoneNumberSearchFilters listFilters) {
        ...
        for (int i = 0; i < phoneNumbers.size(); i++) {
            ...
            if (Integer.parseInt(number.get("stock").toString()) > 0) {
                ...
                final PhoneNumber phoneNumber = new PhoneNumber(friendlyName, number.get("didGroupId").getAsString(), null,
                        null, null, null, null, null, countryCode, null, isVoiceCapable, isSmsCapable, null, isFaxCapable,
                        null);
     ...
     * (non-Javadoc)
     *
     * @see PhoneNumberProvisioningManager#searchForNumbers(java.lang.String, java.lang.String, java.util.regex.Pattern,
    ...
    public List<PhoneNumber> searchForNumbers(String country, PhoneNumberSearchFilters listFilters) {
        ...
        ClientResponse clientResponse = webResource.accept(CONTENT_TYPE).type(CONTENT_TYPE).get(ClientResponse.class);
    ...
    public boolean buyNumber(PhoneNumber phoneNumberObject, PhoneNumberParameters phoneNumberParameters) {
        ...
        try {
            ...
            WebResource addToCartWebResource = addToCartJerseyClient
                    .resource(createCartURI + "/" + cartIdentifier + "/product");
            String addToCartBody = "{\"didCartItem\":{\"didGroupId\":\"" + phoneNumber + "\",\"quantity\":\"1\"}}";
            ClientResponse addToCartResponse = addToCartWebResource.accept(CONTENT_TYPE).type(CONTENT_TYPE)
                    .post(ClientResponse.class, addToCartBody);
            ...
            if (addToCartResponse.getClientResponseStatus() == Status.OK) {
                ...
                if (jsonAddToCartResponse.get("status").getAsString().equalsIgnoreCase("SUCCESS")) {
                    ...
                    WebResource checkoutCartWebResource = checkoutCartJerseyClient
                            .resource(createCartURI + "/" + cartIdentifier + "/checkout");
                    ClientResponse checkoutCartResponse = checkoutCartWebResource.queryParam("cartIdentifier", cartIdentifier)
                    ...
                    if (checkoutCartResponse.getClientResponseStatus() == Status.OK) {
                        ...
                        if (jsonCheckoutCartResponse.get("status").getAsString().equalsIgnoreCase("SUCCESS")) {
                            String orderReference = jsonCheckoutCartResponse.get("productCheckoutList").getAsJsonArray().get(0)
                                    .getAsJsonObject().get("orderReference").getAsString();
                            ...
                            ClientResponse listDidsResponse = listDidsWebResource.queryParam("orderReference", orderReference)
                                    .queryParam(PAGE_NUMBER, "0").queryParam(PAGE_SIZE, "50").accept(CONTENT_TYPE)
                                    .type(CONTENT_TYPE).get(ClientResponse.class);
                ...
                if (logger.isDebugEnabled())
                    logger.debug("Couldn't buy Phone Number " + phoneNumber + ". Response status was: "
                            + addToCartResponse.getClientResponseStatus());
            }
    ...
    public List<String> getAvailableCountries() {
        ...
        ClientResponse clientResponse = webResource.queryParam(PAGE_NUMBER, "0").queryParam(PAGE_SIZE, "300")
                .accept(CONTENT_TYPE).type(CONTENT_TYPE).get(ClientResponse.class);
