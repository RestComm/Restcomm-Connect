...
public final class CallManager extends RestcommUntypedActor {
    ...
    public CallManager(final Configuration configuration, final ServletContext context,
        ...
        if (!runtime.subset("ims-authentication").isEmpty()) {
            ...
            if (actAsImsUa) {
                ...
                if (actAsImsUa
                        && (imsProxyAddress == null || imsProxyAddress.isEmpty() || imsDomain == null || imsDomain.isEmpty())) {
                    logger.warning("ims proxy-address or domain is not configured");
                }
                this.actAsImsUa = actAsImsUa && imsProxyAddress != null && !imsProxyAddress.isEmpty() && imsDomain != null
                        && !imsDomain.isEmpty();
            }
        }
        if (!runtime.subset("acting-as-proxy").isEmpty()
                && !runtime.subset("acting-as-proxy").subset("proxy-rules").isEmpty()) {
            ...
            if (actAsProxyOut) {
                ...
                List<HierarchicalConfiguration> rulesList = ((HierarchicalConfiguration) proxyOutRulesConf)
                        .configurationsAt("rule");
        ...
        if (request == null) {
            props = new Props(new UntypedActorFactory() {
                ...
                public UntypedActor create() throws Exception {
                    return new Call(sipFactory, msControllerFactory, configuration, null, null, null, null);
                }
            });
        } else {
            props = new Props(new UntypedActorFactory() {
                ...
                @Override
                public UntypedActor create() throws Exception {
                    return new Call(sipFactory, msControllerFactory, configuration, request.statusCallback(),
                            request.statusCallbackMethod(), request.statusCallbackEvent(), request.getOutboundProxyHeaders());
                }
            });
        }
    ...
    private void destroy(final Object message) throws Exception {
        ...
        if (call != null) {
            if (logger.isInfoEnabled()) {
                logger.info("About to destroy call: " + request.call().path() + ", call isTerminated(): "
                        + sender().isTerminated() + ", sender: " + sender());
            }
    ...
    private void invite(final Object message) throws IOException, NumberParseException, ServletParseException {
        ...
        if (!request.isInitial()) {
            ...
            if (logger.isInfoEnabled()) {
                logger.info(
                        "No call actor found will respond 200OK for In-dialog INVITE: " + request.getRequestURI().toString());
            }
        ...
        if (client != null) { // make sure the caller is a registered client and not some external SIP agent that we have little
                              // control over
            if (toClient != null) { // looks like its a p2p attempt between two valid registered clients, lets redirect to the
                                    // b2bua
                if (logger.isInfoEnabled()) {
                ...
                IExtensionCreateCallRequest er = new CreateCall(fromUser, toUser, "", "", false, 0, CreateCallType.CLIENT,
                        client.getAccountSid(), null, null, null, null);
                    ...
                    if (B2BUAHelper.redirectToB2BUA(request, client, toClient, storage, sipFactory, patchForNatB2BUASessions)) {
                        if (logger.isInfoEnabled()) {
                            logger.info("Call to CLIENT.  myHostIp: " + myHostIp + " mediaExternalIp: " + mediaExternalIp
                                    + " toHost: " + toHost + " fromClient: " + client.getUri() + " toClient: "
                                    + toClient.getUri());
                        }
                ...
                ExtensionController ec = ExtensionController.getInstance();
                IExtensionCreateCallRequest er = new CreateCall(fromUser, toUser, "", "", false, 0, CreateCallType.PSTN,
                        client.getAccountSid(), null, null, null, null);
                ec.executePreOutboundAction(er, this.extensions);
                if (er.isAllowed()) {
                    if (actAsProxyOut) {
                        ...
                        if (proxyURI != null && !proxyURI.isEmpty()) {
                            ...
                            proxyOut(request, client, toUser, toHost, toHostIpAddress, toPort, outboundIntf, proxyURI,
                                    proxyUsername, proxyPassword, from, to, callToSipUri);
                        } else {
    ...
    /**
     * FIXME: duplicated code make into static function or something more optimized Replace headers
     *
    ...
    private void addHeadersToMessage(SipServletRequest message, Map<String, ArrayList<String>> headers) {
        ...
        if (headers != null) {
            for (Map.Entry<String, ArrayList<String>> entry : headers.entrySet()) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("headerName=" + headerName + " headerVal=" + message.getHeader(headerName) + " concatValue="
                            + sb.toString());
                }
                if (!headerName.equalsIgnoreCase("Request-URI")) {
                    ...
                    for (String keyValPair : entry.getValue()) {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug("ReqURI pars =" + parName + "=" + parVal + " equalsPos=" + equalsPos + " keyValPair="
                                    + keyValPair);
                        }
                    }
    ...
    private boolean proxyOut(SipServletRequest request, Client client, String toUser, String toHost, String toHostIpAddress,
            String toPort, SipURI outboundIntf, String proxyURI, String proxyUsername, String proxyPassword, SipURI from,
            SipURI to, boolean callToSipUri) throws UnknownHostException {
        final Configuration runtime = configuration.subset("runtime-settings");
        ...
        // final String fromPort = String.valueOf(((SipURI) request.getFrom().getURI()).getPort()).equalsIgnoreCase("-1") ?
        // "5060"
        ...
        if (logger.isInfoEnabled()) {
            logger.info("fromHost: " + fromHost + "fromHostIP: " + fromHostIpAddress + "myHostIp: " + myHostIp
                    + " mediaExternalIp: " + mediaExternalIp + " toHost: " + toHost + " toHostIP: " + toHostIpAddress
                    + " proxyUri: " + proxyURI);
        }
        if ((myHostIp.equalsIgnoreCase(toHost) || mediaExternalIp.equalsIgnoreCase(toHost))
            ...
            if (logger.isInfoEnabled()) {
                logger.info("Call to NUMBER.  myHostIp: " + myHostIp + " mediaExternalIp: " + mediaExternalIp + " toHost: "
                        + toHost + " proxyUri: " + proxyURI);
            }
            try {
                if (useLocalAddressAtFromHeader) {
                    if (outboudproxyUserAtFromHeader) {
                        from = (SipURI) sipFactory.createSipURI(proxyUsername, mediaExternalIp + ":" + outboundIntf.getPort());
                    } else {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("Call to SIP URI. myHostIp: " + myHostIp + " mediaExternalIp: " + mediaExternalIp + " toHost: "
                        + toHost + " proxyUri: " + proxyURI);
            }
            from = sipFactory.createSipURI(((SipURI) request.getFrom().getURI()).getUser(),
        ...
        if (B2BUAHelper.redirectToB2BUA(request, client, from, to, proxyUsername, proxyPassword, storage, sipFactory,
                callToSipUri, patchForNatB2BUASessions)) {
            return true;
        }
        return false;
    }
    ...
    private boolean isWebRTC(final SipServletRequest request) {
        ...
        // The check for request.getHeader(UserAgentHeader.NAME).equals("sipunit") has been added in order to be able to test
        // this feature with sipunit at the Restcomm testsuite
    ...
    private void processRequestAndProxyOut(final SipServletRequest request, final Client client, final String destNumber) {
        ...
        if (matchedProxyRule != null) {
            ...
            String rcml;
            if (matchedProxyRule.getUsername() != null && !matchedProxyRule.getUsername().isEmpty()
                    && matchedProxyRule.getPassword() != null && !matchedProxyRule.getPassword().isEmpty()) {
                rcml = String.format("<Response><Dial><Sip username=\"%s\" password=\"%s\">%s</Sip></Dial></Response>",
                        matchedProxyRule.getUsername(), matchedProxyRule.getPassword(), sipUri);
            } else {
    ...
    private void proxyThroughMediaServerAsNumber(final SipServletRequest request, final Client client,
            final String destNumber) {
    ...
    private void proxyDialClientThroughMediaServer(final SipServletRequest request, final Client client,
            final String destNumber) {
    ...
    private void transfer(SipServletRequest request) throws Exception {
        ...
        if (number == null || (number.getReferUrl() == null && number.getReferApplicationSid() == null)) {
            ...
            servletResponse = request.createResponse(SC_NOT_FOUND);
            servletResponse.setHeader("Reason",
                    "SIP Refer failed. Set Refer URL or Refer application for incoming phone number");
            servletResponse.setHeader("Event", "refer");
            servletResponse.send();
            return;
        }
        ...
        if (answer instanceof ActorRef) {
            ...
            servletResponse = request.createResponse(SC_NOT_FOUND);
            servletResponse.setHeader("Reason",
                    "SIP Refer failed. Transferee is not a Call actor, probably this is a conference");
            servletResponse.setHeader("Event", "refer");
            servletResponse.send();
        ...
        builder.setMethod(
                (number.getReferMethod() != null && number.getReferMethod().length() > 0) ? number.getReferMethod() : "POST");
    ...
    private boolean redirectToHostedVoiceApp(final ActorRef self, final SipServletRequest request, final AccountsDao accounts,
        ...
        if (!(phone.contains("*") || phone.contains("#"))) {
            try {
                ...
                /*
                 * if (logger.isInfoEnabled()) { String msg =
                 * String.format("Problem while trying to format number %s, exception %s ",phone, e); logger.info(msg); }
                 */
            }
        }
    ...
    private boolean redirectToClientVoiceApp(final ActorRef self, final SipServletRequest request, final AccountsDao accounts,
        ...
        boolean isClientManaged = ((applicationSid != null && !applicationSid.toString().isEmpty()
                && !applicationSid.toString().equals(""))
                || (clientAppVoiceUrl != null && !clientAppVoiceUrl.toString().isEmpty()
                        && !clientAppVoiceUrl.toString().equals("")));
        if (isClientManaged) {
            final VoiceInterpreterParams.Builder builder = new VoiceInterpreterParams.Builder();
            builder.setConfiguration(configuration);
            builder.setStorage(storage);
            ...
            if (patchForNatB2BUASessions) {
                ...
                if (toInetUri != null && ackRURI == null) {
                    if (isBehindLB) {
                        ...
                        if (patchRURI) {
                            if (logger.isDebugEnabled()) {
                                logger.debug("We are behind load balancer, but Using the real ip address of the sip client "
                            ...
                            // https://github.com/RestComm/Restcomm-Connect/issues/1357
                            if (logger.isDebugEnabled()) {
                                logger.debug("removing the toInetUri to avoid the other subsequent requests using it "
                                        + toInetUri.toString());
                            }
    ...
    private boolean isLBPatchRURI(SipServletRequest request, final String initialIpBeforeLB, String initialPortBeforeLB) {
        try {
            // https://github.com/RestComm/Restcomm-Connect/issues/1336 checking if the initial IP and Port behind LB is part of
            // the route set or not
            ...
            while (routes.hasNext()) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Checking if route " + routeHost + ":" + routePort + " is matching ip and port before LB "
                            + initialIpBeforeLB + ":" + initialPortBeforeLB + " for the " + request.getMethod() + " request");
                }
                if (routeHost.equalsIgnoreCase(initialIpBeforeLB) && routePort == Integer.parseInt(initialPortBeforeLB)) {
                    if (logger.isDebugEnabled()) {
                        logger.debug("route " + route + " is matching ip and port before LB " + initialIpBeforeLB + ":"
                                + initialPortBeforeLB + " for the " + request.getMethod()
                                + " request, so not patching the Request-URI");
                    }
                    return false;
                }
            }
    ...
    private void update(final Object message) throws Exception {
        ...
        // TODO possible bug here. Since we have more than one call observer, later there might be the case that the first one
        // is not the VI
        // TODO set the VI using specific message, also get the VI using specific message. The VI will still be in the observers
        // list but it will set/get using specific method
        ActorRef existingInterpreter = callObservers.iterator().next();
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("LCM account: " + request.account() + ", moveConnectedCallLeg: " + moveConnectedCallLeg
                    + ", new RCML url: " + request.url());
        }
        ...
        if (relatedCall != null && listOfRelatedCalls == null) {
            if (moveConnectedCallLeg) {
                ...
                if (logger.isInfoEnabled()) {
                    logger.info("moveConnectedCallLeg is: " + moveConnectedCallLeg + " so will hangup relatedCall: "
                            + relatedCall.path());
                }
        ...
        if (listOfRelatedCalls != null) {
            ...
            if (logger.isInfoEnabled()) {
                String msg = String.format("LiveCallModification request while dial forking, terminated %d calls",
                        listOfRelatedCalls.size());
                logger.info(msg);
            }
        }
    }
        ...
        switch (request.type()) {
            case CLIENT: {
                if (request.isAllowed()) {
                    ...
                    sender.tell(
                            new CallManagerResponse<ActorRef>(new RestcommExtensionException(errMsg), this.createCallRequest),
                            self());
                }
                    ...
                    if (registration.isLBPresent()) {
                        if (logger.isInfoEnabled())
                            logger.info("WebRTC registration behind LB. Will add WebRTC registration: "
                                    + registration.getLocation() + " to the list to be dialed for client: " + client);
                        registrationToDial.add(registration);
                    } else {
                        // If this is a WebRTC client registration, check that the InstanceId of the registration is for the
                        // current Restcomm instance
        ...
        if (registrationToDial.size() > 0) {
            if (logger.isInfoEnabled()) {
                if (registrationToDial.size() > 1) {
                    logger.info("Preparing call for client: " + client + ", after WebRTC check, Restcomm have to dial :"
            ...
            for (Registration registration : registrationToDial) {
                ...
                if (outboundIntf == null) {
                    String errMsg = "The outbound interface for transport: " + transport
                            + " is NULL, something is wrong with container, cannot proceed to call client " + request.to();
                    logger.error(errMsg);
                    sendNotification(errMsg, 11008, "error", true);
                ...
                if (request.from() != null && request.from().contains("@")) {
                    ...
                    if (outboundIntf != null) {
                        ...
                        logger.error(
                                "Outbound interface is null, cannot create From header to be used to Dial client: " + client);
                    }
                } else {
                    from = outboundIntf;
                }
    ...
    private void outboundToPstn(final CreateCall request, final ActorRef sender) throws ServletParseException {
        final String uri = (request.getOutboundProxy() != null && (!request.getOutboundProxy().isEmpty()))
                ? request.getOutboundProxy() : activeProxy;
        SipURI outboundIntf = null;
        SipURI from = null;
        SipURI to = null;
    ...
    public void cancel(final Object message) throws IOException {
        ...
        if (originalRequest != null) {
            ...
            SipServletResponse lastFinalResponse = (SipServletResponse) originalRequest.getSession()
                    .getAttribute(B2BUAHelper.B2BUA_LAST_FINAL_RESPONSE);

            if ((sessionState == SipSession.State.INITIAL.name() || sessionState == SipSession.State.EARLY.name())
                    && !(lastFinalResponse != null
                            && (lastFinalResponse.getStatus() == 401 || lastFinalResponse.getStatus() == 407))) {
            ...
            if (patchForNatB2BUASessions) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("toInetUri: " + toInetUri + " fromInetUri: " + fromInetUri + " byeRURI: " + byeRURI
                            + " initialIpBeforeLB: " + initialIpBeforeLB + " initialPortBeforeLB: " + initialPortBeforeLB);
                }
    ...
    private void imsProxyThroughMediaServer(final SipServletRequest request, final Client client,
        ...
        if (logger.isInfoEnabled()) {
            logger.info(
                    "imsProxyThroughMediaServer, isFromIms: " + isFromIms + ", destUri: " + destUri + ", srcUri: " + srcUri);
        }
        final Configuration runtime = configuration.subset("runtime-settings");
    ...
    private void outboundToIms(final CreateCall request, final ActorRef sender) throws ServletParseException {
        ...
        if (from == null || to == null) {
            ...
            InitializeOutbound init = new InitializeOutbound(request.from(), from, to, proxyUsername, proxyPassword,
                    request.timeout(), request.isFromApi(), runtime.getString("api-version"), request.accountId(),
                    request.type(), storage, isToWebRTC, true, imsProxyAddress, imsProxyPort);
            if (request.parentCallSid() != null) {
                init.setParentCallSid(request.parentCallSid());
            }
            call.tell(init, self);
