...
public final class UserAgentManager extends RestcommUntypedActor {
            ...
            if (result.isWebRTC()) {
                ...
                if (expires.isBeforeNow() || expires.isEqualNow()) {
                    ...
                    if ((DateTime.now().getMillis() - updated.getMillis()) > pingIntervalMillis) {
                        // Last time this registration updated was older than (pingInterval * 3), looks like it doesn't respond
                        // to OPTIONS
                        if (logger.isInfoEnabled()) {
                            logger.info("Registration: " + result.getLocation()
                                    + " didn't respond to OPTIONS and will be removed now");
                        }
                        registrations.removeRegistration(result);
                        monitoringService.tell(new UserRegistration(result.getUserName(), result.getLocation(), false), self());
                        monitoringService.tell(new GetCall(result.getLocation()), self());
    ...
    private void clean() throws ServletException {
        ...
        for (final Registration result : results) {
            ...
            if (expires.isBeforeNow() || expires.isEqualNow()) {
                ...
                Long pingIntervalMillis = new Long(pingInterval * 1000 * 3);
                if ((DateTime.now().getMillis() - updated.getMillis()) > pingIntervalMillis) {
                    // Last time this registration updated was older than (pingInterval * 3), looks like it doesn't respond to
                    // OPTIONS
                    if (logger.isInfoEnabled()) {
                        logger.info("Registration: " + result.getAddressOfRecord()
                                + " didn't respond to OPTIONS. Will ping again.");
                    }
                    // Instead of removing registrations we ping the client one last time to ensure it was not a temporary loss
                    // of connectivity. We don't need to remove the registration here. It will be handled only if the OPTIONS
                    // ping
                    // times out and the related calls from the client cleaned up as well
                    try {
                        ping(result.getLocation());
    ...
    private void keepAlive() throws Exception {
        ...
        if (results != null && results.size() > 0) {
            if (logger.isDebugEnabled()) {
                logger.debug(
                        "Registrations for InstanceId: " + instanceId + " , returned " + results.size() + " registrations");
            }
            for (final Registration result : results) {
    ...
    public void onReceive(final Object message) throws Exception {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("UserAgentManager Processing Message: \"" + klass.getName() + " sender : " + sender.getClass()
                    + " self is terminated: " + self().isTerminated());
        }
        if (message instanceof ReceiveTimeout) {
            if (logger.isDebugEnabled()) {
                logger.debug("Timeout received, ping interval: " + pingInterval
                        + " , will clean up registrations and send keep alive");
            }
            clean();
            keepAlive();
            ...
            if ("REGISTER".equalsIgnoreCase(method)) {
                ...
                } else if (authenticateUsers) { // https://github.com/Mobicents/RestComm/issues/29 Allow disabling of SIP
                                                // authentication
                    ...
                    if (authorization != null) {
                        if (permitted(authorization, method)) {
                            ...
                            SipServletResponse response = ((SipServletRequest) message)
                                    .createResponse(javax.servlet.sip.SipServletResponse.SC_FORBIDDEN); // Issue #935, Send 403
                                                                                                        // FORBIDDEN instead of
                                                                                                        // issuing 407 again and
                                                                                                        // again
                            response.send();
                        }
                    } else {
    ...
    private void removeRegistration(final SipServletMessage sipServletMessage, boolean locationInContact)
        ...
        if (registrations != null) {
            ...
            while (iter.hasNext()) {
                ...
                // Long pingIntervalMillis = new Long(pingInterval * 1000 * 3);
                // boolean optionsTimeout = ((DateTime.now().getMillis() - reg.getDateUpdated().getMillis()) >
                // pingIntervalMillis);
                ...
                // We clean up only if the location is similar to the registration location to avoid cleaning up all
                // registration lcoation for the AOR
                ...
                if (locationStored != null && locationStored.equalsIgnoreCase(locationToTest)) {
                    // && (reg.getAddressOfRecord().equalsIgnoreCase(regLocation.toString()) ||
                    // reg.getLocation().equalsIgnoreCase(regLocation.toString()))) {

                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "Registration: " + reg.getLocation() + " failed to response to OPTIONS and will be removed");
                    }
                    ...
                    if (logger.isDebugEnabled()) {
                        String msg = String.format(
                                "Registration DID NOT removed. SIP Message location: %s, registration location (in DB) %s.",
                                locationToTest, reg.getLocation());
                        logger.debug(msg);
                    }
                }
            }
    ...
    private void ping(final String to) throws ServletException {
        ...
        /*
         * sometime users on webrtc clients like olympus, closes the tab instead of properly hangup. so removing this check so
         * we could send OPTIONS to WebRtc clients as well: if (toTransport.equalsIgnoreCase("ws") ||
         * toTransport.equalsIgnoreCase("wss")) { return; }
         */
        ...
        try {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("There was a problem while trying to ping client: " + to + " , will remove registration. "
                        + e.getMessage());
            }
    ...
    private void register(final Object message) throws Exception {
        ...
        String transport = (uri.getTransportParam() == null ? request.getParameter("transport") : uri.getTransportParam()); // Issue
                                                                                                                            // #935,
                                                                                                                            // take
                                                                                                                            // transport
                                                                                                                            // of
        ...
        if (initialIpBeforeLB != null && !initialIpBeforeLB.isEmpty() && initialPortBeforeLB != null
            ...
            if (logger.isInfoEnabled()) {
                logger.info(
                        "Patching URI: " + uri.toString() + " with IP: " + ip + " and PORT: " + port + " for USER: " + user);
            }
            patch(uri, ip, port);
        }
        ...
        final Registration registration = new Registration(sid, instanceId, now, now, aor, name, user, ua, ttl, address, webRTC,
                isLBPresent);
        final RegistrationsDao registrations = storage.getRegistrationsDao();
    ...
    private boolean isWebRTC(String transport, String userAgent) {
        return "ws".equalsIgnoreCase(transport) || "wss".equalsIgnoreCase(transport)
                || userAgent.toLowerCase().contains("restcomm");
    }
    ...
    private void proxyResponseFromIms(Object message, SipServletResponse response) throws ServletParseException, IOException {
        ...
        final SipServletRequest incomingRequest = (SipServletRequest) response.getApplicationSession()
                .getAttribute(REQ_PARAMETER);
        ...
        SipServletResponse incomingLegResposne = incomingRequest.createResponse(response.getStatus(),
        ...
        if (response.getStatus() >= 400 && response.getStatus() != SC_UNAUTHORIZED
                && response.getStatus() != SC_PROXY_AUTHENTICATION_REQUIRED) {
            removeRegistration(incomingRequest, true);
        } else if (response.getStatus() == 200) {
            String transport = (uri.getTransportParam() == null ? incomingRequest.getParameter("transport")
                    : uri.getTransportParam()); // Issue #935, take transport of initial request-uri if contact-uri has no
                                                // transport parameter
            ...
            if (ua == null)
                ua = "GenericUA";
            boolean webRTC = isWebRTC(transport, ua);
            final Registration registration = new Registration(sid,
                    RestcommConfiguration.getInstance().getMain().getInstanceId(), now, now, aor, name, user, ua, ttl, address,
                    webRTC, isLBPresent);
            final RegistrationsDao registrations = storage.getRegistrationsDao();
    ...
    private void proxyRequestToIms(SipServletRequest request) throws ServletParseException, IOException {
        // TODO question - this method is deprecated but only that can be used to set callId which has to be the same for both
        // REGISTER messages
    ...
    public void postStop() {
        try {
            if (logger.isInfoEnabled()) {
                logger.info("UserAgentManager actor at postStop, path: " + self().path() + ", isTerminated: "
                        + self().isTerminated() + ", sender: " + sender());
