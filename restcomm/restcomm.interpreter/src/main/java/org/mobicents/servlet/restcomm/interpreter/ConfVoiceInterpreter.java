...
public class ConfVoiceInterpreter extends UntypedActor {
	...
	public ConfVoiceInterpreter(final Configuration configuration, final Sid account, final String version,
			final URI url, final String method, final String emailAddress, final ActorRef conference,
			final DaoManager storage, final CallInfo callInfo) {
		...
		acquiringConfMediaGroup = new State("acquiring call media group", new AcquiringConferenceMediaGroup(source),
				null);
		downloadingRcml = new State("downloading rcml", new DownloadingRcml(source), null);
		initializingConfMediaGroup = new State("initializing call media group", new InitializingConferenceMediaGroup(
				source), null);
		...
		if (response != null) {
			...
			if (type.contains("text/xml") || type.contains("application/xml") || type.contains("text/html")) {
				try {
					...
					logger.error(
							"There was an error while reading the contents of the resource " + "located @ "
	...
	public void onReceive(final Object message) throws Exception {
		...
		if (StartInterpreter.class.equals(klass)) {
			...
			} else if (downloaderResponse.succeeded()
					&& HttpStatus.SC_NOT_FOUND == downloaderResponse.get().getStatusCode()) {
	...
	private ActorRef tts(final Configuration configuration) {
		...
		return context.actorOf(new Props(new UntypedActorFactory() {
			...
			public Actor create() throws Exception {
				return (UntypedActor) Class.forName(classpath).getConstructor(Configuration.class)
	...
	private final class NotFound extends AbstractAction {
		...
		public void execute(final Object message) throws Exception {
			...
			if (logger.isDebugEnabled()) {
				logger.debug("response succeeded " + response.succeeded() + ", statusCode "
			...
			if (SpeechSynthesizerResponse.class.equals(klass)) {
				...
				if (text != null && !text.isEmpty()) {
					...
					try {
						...
						final Notification notification = notification(ERROR_NOTIFICATION, 11100, text
								+ " is an invalid URI.");
						final NotificationsDao notifications = storage.getNotificationsDao();
						notifications.addNotification(notification);
					...
					if (number != null && !number.isEmpty()) {
						try {
							...
							if (say.equals(verb.name())) {
								notification = notification(WARNING_NOTIFICATION, 13510, loop
										+ " is an invalid loop value.");
								notifications.addNotification(notification);
							} else if (play.equals(verb.name())) {
								notification = notification(WARNING_NOTIFICATION, 13410, loop
										+ " is an invalid loop value.");
								notifications.addNotification(notification);
							}
						}
