...
public final class VoiceInterpreter extends BaseVoiceInterpreter {
    ...
    private void onJoinComplete(JoinComplete message)
            throws TransitionNotFoundException, TransitionFailedException, TransitionRollbackException {
        if (logger.isInfoEnabled()) {
    ...
    private void onAsrResponse(Object message)
    ...
    private void onForkMessage(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onConferenceCenterResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onConferenceResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        ...
        if (Left.class.equals(klass)) {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("VoiceInterpreter received ConferenceResponse from Conference: " + conferenceInfo.name()
                        + ", path: " + sender().path() + ", current confernce size: " + conferenceInfo.globalParticipants()
                        + ", VI state: " + fsm.state());
            }
    ...
    private void onConferenceStateChanged(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        ...
        switch (event.state()) {
            ...
            case STOPPING:
                ...
                if (is(joiningConference)) {
                    if (logger.isInfoEnabled()) {
                        logger.info(
                                "We tried to join a stopping conference. Will ask Conference Center to create a new conference for us.");
                    }
    ...
    private void onParserFailed(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        if (logger.isInfoEnabled()) {
    ...
    private void onStopInterpreter(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        this.liveCallModification = ((StopInterpreter) message).isLiveCallModification();
        if (logger.isInfoEnabled()) {
            String msg = String.format("Got StopInterpreter, liveCallModification %s, CallState %s", liveCallModification,
                    callState);
            logger.info(msg);
        }
    ...
    private void onEmailResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onSmsServiceResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onMediaGroupResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onEndMessage(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        ...
        if (playWaitUrlPending && conferenceWaitUris != null && conferenceWaitUris.size() > 0) {
            if (logger.isInfoEnabled()) {
                String msg = String.format("End tag received, playWaitUrlPending is %s, conferenceWaitUris.size() %d",
                        playWaitUrlPending, conferenceWaitUris.size());
                logger.info(msg);
            }
        ...
        if (callState.equals(CallStateChanged.State.COMPLETED) || callState.equals(CallStateChanged.State.CANCELED)) {
            if (logger.isInfoEnabled()) {
                String msg = String.format("End tag received, Call state %s , VI state %s will move to finished state",
                        callState, fsm.state());
                logger.info(msg);
            }
            fsm.transition(message, finished);
            ...
            if (!isParserFailed) {
                ...
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "End tag received but parser failed earlier so hangup would have been already sent to the call");
                }
            }
        }
    }

    private void onTagMessage(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        verb = (Tag) message;
        if (logger.isDebugEnabled()) {
    ...
    private void onDiskCacheResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onCallManagerResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        ...
        if (response.succeeded()) {
            ...
            if (logger.isDebugEnabled()) {
                String msg = String.format(
                        "CallManager failed to create Call for %s, current state %s, dialChilder.size %s, dialBranches.size %s",
                        response.getCreateCall().to(), fsm.state().toString(), dialChildren.size(), dialBranches.size());
                logger.debug(msg);
            }
    ...
    private void onSpeechSynthesizerResponse(Object message)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onCallResponse(Object message, State state)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        if (forking.equals(state)) {
            ...
            if (sender == call) {
                callInfo = response.get();
                if (callInfo.state() == CallStateChanged.State.CANCELED || (callInfo.invite() != null
                        && callInfo.invite().getSession().getState().equals(SipSession.State.TERMINATED))) {
                    fsm.transition(message, finished);
                    return;
                } else {
    ...
    private void onDownloaderResponse(Object message, State state)
            throws IOException, TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
    ...
    private void onCallStateChanged(Object message, ActorRef sender)
            throws TransitionFailedException, TransitionNotFoundException, TransitionRollbackException {
        ...
        if (logger.isInfoEnabled()) {
            logger.info("VoiceInterpreter received CallStateChanged event: " + event + " from "
                    + (sender == call ? "call" : "outboundCall") + ", sender path: " + sender.path() + ", current VI state: "
                    + fsm.state());
        }
        ...
        switch (event.state()) {
            ...
            case NO_ANSWER:
                ...
                if (is(bridging) || (is(bridged) && !sender.equals(call))) {
                    ...
                    if ((dialBranches == null || dialBranches.size() == 0) && sender.equals(call)) {
                        // TODO HERE
                        logger.info("No-Answer event received, and dialBrances is either null or 0 size, sender: "
                                + sender.path() + ", vi state: " + fsm.state());
                        checkDialBranch(message, sender, action);
                    }
                }
                if (is(initializingCall)) {
            ...
            case COMPLETED:
                ...
                if (logger.isInfoEnabled()) {
                    String msg = String.format("OnCallStateChanged, VI state %s, received %s, is it from inbound call: %s",
                            fsm.state().toString(), callState.toString(), sender.equals(call));
                    logger.info(msg);
                }
                if (is(bridging)) {
                    fsm.transition(message, finishDialing);
                ...
                if (is(forking) && ((dialBranches != null && dialBranches.contains(sender)) || outboundCall == null)) {
                    ...
                    // event, then return and wait for the FinishConferencing to deal with the event (either execute dial action
                    // or
                    // get next verb from parser
                    if (logger.isInfoEnabled()) {
                        logger.info("VoiceInterpreter received CallStateChanged.Completed VI in: " + fsm.state()
                                + " state, will return and wait for ConferenceStateChanged.Completed event");
                    }
                    return;
                } else {
                    if (!is(finishDialing))
    ...
    private void removeDialBranch(Object message, ActorRef sender) {
        ...
        if (logger.isInfoEnabled()) {
            logger.info(
                    "Dial branch new call state: " + state + " call path: " + sender().path() + " VI state: " + fsm.state());
        }
        if (state != null && !state.equals(CallStateChanged.State.CANCELED)) {
            if (logger.isInfoEnabled()) {
                logger.info("At removeDialBranch() will cancel call: " + sender.path() + ", isTerminated: "
                        + sender.isTerminated());
            }
            sender.tell(new Cancel(), self());
        }
    ...
    private void checkDialBranch(Object message, ActorRef sender, Attribute attribute) {
        ...
        } else if (state != null && (state.equals(CallStateChanged.State.BUSY) || state.equals(CallStateChanged.State.CANCELED)
    ...
    List<NameValuePair> parameters() {
        ...
        final String forwardedFrom = (callInfo.forwardedFrom() == null || callInfo.forwardedFrom().isEmpty()) ? "null"
                : callInfo.forwardedFrom();
    ...
    private abstract class AbstractDialAction extends AbstractAction {
        ...
        protected String callerId(final Tag container) {
            ...
            final boolean useInitialFromAsCallerId = configuration.subset("runtime-settings")
                    .getBoolean("from-address-to-proxied-calls");
            ...
            if (attribute != null) {
                ...
                if (value != null && !value.isEmpty()) {
                    try {
                        ...
                        final NotificationsDao notifications = storage.getNotificationsDao();
                        final Notification notification = notification(WARNING_NOTIFICATION, 13212,
                                value + " is not a valid timeout value for <Dial>");
                        notifications.addNotification(notification);
                    }
                }
            }
            ...
            if (attribute != null) {
                ...
                if (value != null && !value.isEmpty()) {
                    try {
                        ...
                        final NotificationsDao notifications = storage.getNotificationsDao();
                        final Notification notification = notification(WARNING_NOTIFICATION, 13216,
                                value + " is not a valid timeLimit value for <Dial>");
                        notifications.addNotification(notification);
                    }
                }
            }
                ...
                if (child != null) {
                    ...
                    if (logger.isInfoEnabled()) {
                        logger.info("Dial verb " + verb.toString().replace("\\n", "")
                                + " with more that one element, will start forking. Dial Children size: "
                                + dialChildren.size());
                    }
                }
            } else {
            ...
            if (!dialChildren.isEmpty()) {
                ...
                if (statusCallback != null) {
                    ...
                    if (child.hasAttribute("statusCallbackEvent")) {
                        statusCallbackEvent = Arrays
                                .asList(child.attribute("statusCallbackEvent").value().replaceAll("\\s+", "").split(","));
                    } else {
                ...
                if (Nouns.client.equals(child.name())) {
                    if (call != null && callInfo != null) {
                        create = new CreateCall(e164(callerId(verb)), e164(child.text()), null, null, callInfo.isFromApi(),
                                timeout(verb), CreateCallType.CLIENT, accountId, callInfo.sid(), statusCallback,
                                statusCallbackMethod, statusCallbackEvent);
                    } else {
                    ...
                    if (call != null && callInfo != null) {
                        create = new CreateCall(e164(callerId(verb)), e164(child.text()), null, null, callInfo.isFromApi(),
                                timeout(verb), CreateCallType.PSTN, accountId, callInfo.sid(), statusCallback,
                                statusCallbackMethod, statusCallbackEvent);
                    } else {
                        create = new CreateCall(e164(callerId(verb)), e164(child.text()), null, null, false, timeout(verb),
                    ...
                    if (call != null && callInfo != null) {
                        create = new CreateCall(e164(callerId(verb)), e164(child.text()), null, null, callInfo.isFromApi(),
                                timeout(verb), CreateCallType.SIP, accountId, callInfo.sid(), statusCallback,
                                statusCallbackMethod, statusCallbackEvent);
                    } else {
                        create = new CreateCall(e164(callerId(verb)), e164(child.text()), null, null, false, timeout(verb),
                    ...
                    if (call != null && callInfo != null) {
                        create = new CreateCall(e164(callerId(verb)), e164(child.text()), username, password, false,
                                timeout(verb), CreateCallType.SIP, accountId, callInfo.sid(), statusCallback,
                                statusCallbackMethod, statusCallbackEvent);
                    } else {
                        create = new CreateCall(e164(callerId(verb)), e164(child.text()), username, password, false,
                                timeout(verb), CreateCallType.SIP, accountId, null, statusCallback, statusCallbackMethod,
                                statusCallbackEvent);
                    }
                }
        ...
        if (!dialActionExecuted && verb != null && Verbs.dial.equals(verb.name())) {
            ...
            if (outboundCall != null && !outboundCall.isTerminated()) {
                try {
                    ...
                    final long dialRingDuration = new Interval(this.outboundCallInfo.dateCreated(),
                            this.outboundCallInfo.dateConUpdated()).toDuration().getStandardSeconds();
            ...
            if (message instanceof CallManagerResponse && !(((CallManagerResponse<ActorRef>) message).succeeded())) {
                if (outboundCallInfo != null) {
                    parameters.add(new BasicNameValuePair("DialCallSid",
                            (outboundCallInfo.sid() == null) ? "null" : outboundCallInfo.sid().toString()));
                } else {
            ...
            // Handle No-Answer calls
                ...
                if (outboundCallInfo != null) {
                    ...
                    final String publicRecordingUrl = this.publicRecordingUri == null ? null
                            : this.publicRecordingUri.toString();
                ...
                if (outboundCallInfo != null) {
                    ...
                    // In some cases, such as when the outbound dial is busy, the dialCallDuration wont be possbile to be
                    // calculated and will throw exception
                    try {
                        dialCallDuration = new Interval(this.outboundCallInfo.dateConUpdated(), DateTime.now()).toDuration()
                                .getStandardSeconds();
                    } catch (Exception e) {
                    ...
                    if (callInfo.state().equals(CallStateChanged.State.COMPLETED)) {
                        ...
                        parameters.add(new BasicNameValuePair("DialCallStatus",
                                dialCallStatus == null ? null : dialCallStatus.toString()));
                    }
            ...
            if (attribute != null) {
                ...
                if (action != null && !action.isEmpty()) {
                    ...
                    if (attribute != null) {
                        ...
                        if (method != null && !method.isEmpty()) {
                            if (!"GET".equalsIgnoreCase(method) && !"POST".equalsIgnoreCase(method)) {
                                final Notification notification = notification(WARNING_NOTIFICATION, 13210,
                                        method + " is not a valid HTTP method for <Dial>");
                                notifications.addNotification(notification);
                                method = "POST";
                            }
                        } else {
                    ...
                    // Tell the downloader to send the Dial Parameters to the Action url but we don't need a reply back so
                    // sender == null
    ...
    private final class FinishDialing extends AbstractDialAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            if (message instanceof CallStateChanged) {
                if (logger.isInfoEnabled()) {
                    logger.info("CallStateChanged state: " + ((CallStateChanged) message).state().toString() + " ,sender: "
                            + sender().path());
                }
                if (forking.equals(state) || finishDialing.equals(state) || is(bridged) || is(bridging)) {
                    if (sender.equals(call)) {
                        ...
                        // to completed state because checkDialBranch() method will ask for the next verb which could be the
                        // End.tag
                        ...
                        if (attribute == null) {
                            if (logger.isInfoEnabled()) {
                                logger.info(
                                        "At FinishDialing. Sender NOT in the dialBranches, attribute is null, will check for the next verb");
                            }
                            final GetNextVerb next = new GetNextVerb();
                            if (parser != null) {
                                parser.tell(next, source);
                            ...
                            if (logger.isInfoEnabled()) {
                                logger.info(
                                        "At FinishDialing. Sender NOT in the dialBranches, attribute is NOT null, will execute Dial Action");
                            }
                            executeDialAction(message, outboundCall);
                        }
                        dialChildren = null;
    ...
    private final class JoiningConference extends AbstractDialAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            if (conferenceInfo.globalParticipants() < maxParticipantLimit) {
                ...
                if (beep && ConferenceStateChanged.State.RUNNING_MODERATOR_PRESENT.equals(conferenceInfo.state())) {
                    ...
                    if (logger.isInfoEnabled()) {
                        logger.info(
                                "Wont play beep bcz: beep=" + beep + " AND conferenceInfo.state()=" + conferenceInfo.state());
                    }
                }
                if (logger.isInfoEnabled()) {
                    logger.info("About to join call to Conference: " + conferenceInfo.name() + ", with state: "
                            + conferenceInfo.state() + ", with moderator present: " + conferenceInfo.isModeratorPresent()
                            + ", and current participants: " + conferenceInfo.globalParticipants());
                }
                // Join the conference.
        ...
        private void addConferenceStuffInCDR(Sid conferenceSid) {
            ...
            if (callRecord != null) {
                if (logger.isInfoEnabled()) {
                    logger.info("Updating CDR for call: " + callInfo.sid() + ", call status: " + callInfo.state()
    ...
    private final class Conferencing extends AbstractDialAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            if (logger.isInfoEnabled()) {
                logger.info("At conferencing, VI state: " + fsm.state() + " , playMusicForConference: " + playMusicForConference
                        + " ConferenceState: " + conferenceState.name() + " startConferenceOnEnter: " + startConferenceOnEnter
                        + "  conferenceInfo.globalParticipants(): " + conferenceInfo.globalParticipants());
            }
            ...
            if (!startConferenceOnEnter && conferenceState == ConferenceStateChanged.State.RUNNING_MODERATOR_ABSENT) {
                ...
                // updateMuteAndHoldStatusOfAllConferenceCalls(conferenceDetailRecord.getAccountSid(),
                // conferenceDetailRecord.getSid(), false, false);
            } else {
                // Call is no more on hold
                // open this block when mute/hold functionality in conference API is fixed
                // updateMuteAndHoldStatusOfAllConferenceCalls(conferenceDetailRecord.getAccountSid(),
    ...
    /*
     * open this block when mute/hold functionality in conference API is fixed protected void
     * updateMuteAndHoldStatusOfAllConferenceCalls(final Sid accountSid, final Sid conferenceSid, final boolean mute, final
     * boolean hold) throws ParseException{ if (conferenceSid != null){ CallDetailRecordFilter filter = new
     * CallDetailRecordFilter(accountSid.toString(), null, null, null, "in-progress", null, null, null,
     * conferenceSid.toString(), 50, 0); CallDetailRecordsDao callRecordsDAO = storage.getCallDetailRecordsDao();
     * List<CallDetailRecord> conferenceCallRecords = callRecordsDAO.getCallDetailRecords(filter); if(conferenceCallRecords !=
     * null){ for(CallDetailRecord singleRecord:conferenceCallRecords){ singleRecord.setMuted(mute);
     * singleRecord.setOnHold(hold); callRecordsDAO = storage.getCallDetailRecordsDao();
    ...
    private final class FinishConferencing extends AbstractDialAction {
        ...
        public void execute(final Object message) throws Exception {
            if (message instanceof ReceiveTimeout) {
                if (logger.isInfoEnabled()) {
                    logger.info("At FinishConferencing received timeout, VI path: " + self().path() + ", call path: "
                            + call.path());
                }
                final UntypedActorContext context = getContext();
                context.setReceiveTimeout(Duration.Undefined());
    ...
    private final class Finished extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            // // Do not play beep if this was the last participant to leave the conference, because there is no one to listen
            // to the beep.
