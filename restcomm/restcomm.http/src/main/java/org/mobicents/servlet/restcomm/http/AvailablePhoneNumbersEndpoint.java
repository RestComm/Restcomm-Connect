...
public abstract class AvailablePhoneNumbersEndpoint extends AbstractEndpoint {
    ...
    public void init() throws ObjectInstantiationException {
        ...
        phoneNumberProvisioningManager = (PhoneNumberProvisioningManager) 
        		new ObjectFactory(getClass().getClassLoader()).getObjectInstance(phoneNumberProvisioningManagerClass);
    ...
    protected Response getAvailablePhoneNumbers(final String accountSid, final String isoCountryCode,
            String areaCode, String filterPattern, boolean smsEnabled, boolean mmsEnabled, boolean voiceEnabled, boolean faxEnabled, 
            int rangeSize, int rangeIndex, final MediaType responseType) {
    	final List<PhoneNumber> phoneNumbers = phoneNumberProvisioningManager.searchForNumbers(
    			isoCountryCode, areaCode, filterPattern, smsEnabled, mmsEnabled, voiceEnabled, faxEnabled, rangeSize, rangeIndex);
...
//    }
	...
	private List<AvailablePhoneNumber> toAvailablePhoneNumbers(
		List<PhoneNumber> phoneNumbers) {
        final List<AvailablePhoneNumber> availablePhoneNumbers = new ArrayList<AvailablePhoneNumber>();
        for (PhoneNumber phoneNumber : phoneNumbers) {
        	final AvailablePhoneNumber number = new AvailablePhoneNumber(phoneNumber.getFriendlyName(), phoneNumber.getPhoneNumber(),
                    phoneNumber.getLata(), phoneNumber.getRateCenter(), phoneNumber.getLatitude(), phoneNumber.getLongitude(), phoneNumber.getRegion(), phoneNumber.getPostalCode(), 
                    phoneNumber.getIsoCountry(), phoneNumber.isVoiceCapable(), phoneNumber.isSmsCapable(), phoneNumber.isMmsCapable(), phoneNumber.isFaxCapable());
