    ...
    protected Response getCalls(final String accountSid, final String conferenceSid, UriInfo info, MediaType responseType) {
        ...
        try {
            ...
            if (localInstanceOnly) {
                filterForTotal = new CallDetailRecordFilter(accountSid, null, null, null, status, null, null, null,
                        conferenceSid, null, null);
            } else {
                filterForTotal = new CallDetailRecordFilter(accountSid, null, null, null, status, null, null, null,
                        conferenceSid, null, null, instanceId);
            }
        ...
        try {
            if (localInstanceOnly) {
                filter = new CallDetailRecordFilter(accountSid, null, null, null, status, null, null, null, conferenceSid,
                        limit, offset);
            } else {
                filter = new CallDetailRecordFilter(accountSid, null, null, null, status, null, null, null, conferenceSid,
        ...
        if (logger.isDebugEnabled()) {
            ...
            for (CallDetailRecord cdr : allCdrs) {
                logger.debug("CDR sid: " + cdr.getSid() + ", status: " + cdr.getStatus() + ", conferenceSid: "
                        + cdr.getConferenceSid());
            }
        }
