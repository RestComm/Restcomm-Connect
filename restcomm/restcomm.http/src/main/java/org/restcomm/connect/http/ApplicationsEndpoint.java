...
public class ApplicationsEndpoint extends SecuredEndpoint {
    ...
    protected Response getApplication(final String accountSid, final String sid, final MediaType responseType) {
        ...
        if (Sid.pattern.matcher(sid).matches()) {
           ...
           * else { // disabled support for application retrieval based on FriendlyName. It makes no sense to have it if
           * friendly-name based application uniqueness is no longer supported either.
           *
           * try { // Once not a valid sid, search using the parameter as name String name =
           * URLDecoder.decode(String.valueOf(sid), "UTF-8"); application = dao.getApplication(name); } catch
           * (UnsupportedEncodingException e) { return status(BAD_REQUEST).entity(e.getMessage()).build(); } }
           */
        if (application == null) {
    ...
    public Response putApplication(final String accountSid, final MultivaluedMap<String, String> data,
        ...
        // .entity("A application with the same name was already created by another account. Please, choose a different name and
        // try again.")
        // .build();
        // }
