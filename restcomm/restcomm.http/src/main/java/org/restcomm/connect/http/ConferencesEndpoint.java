...
public abstract class ConferencesEndpoint extends SecuredEndpoint {
    ...
    protected Response getConferences(final String accountSid, UriInfo info, MediaType responseType) {
        ...
        try {
            filterForTotal = new ConferenceDetailRecordFilter(accountSid, status, dateCreated, dateUpdated, friendlyName, null,
                    null);
        ...
        try {
            filter = new ConferenceDetailRecordFilter(accountSid, status, dateCreated, dateUpdated, friendlyName, limit,
                    offset);
        } catch (ParseException e) {
            return status(BAD_REQUEST).build();
        }
