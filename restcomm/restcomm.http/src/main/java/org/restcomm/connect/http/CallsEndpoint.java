...
public abstract class CallsEndpoint extends SecuredEndpoint {
    ...
    protected Response getCalls(final String accountSid, UriInfo info, MediaType responseType) {
        ...
        try {
            ...
            if (localInstanceOnly) {
                filterForTotal = new CallDetailRecordFilter(accountSid, ownerAccounts, recipient, sender, status, startTime,
                        endTime, parentCallSid, conferenceSid, null, null);
            } else {
                filterForTotal = new CallDetailRecordFilter(accountSid, ownerAccounts, recipient, sender, status, startTime,
                        endTime, parentCallSid, conferenceSid, null, null, instanceId);
            }
     ...
     */
    protected void muteUnmuteCall(Boolean mute, CallInfo callInfo, ActorRef call, CallDetailRecord cdr,
            CallDetailRecordsDao dao) {
        if (callInfo.state().name().equalsIgnoreCase("IN_PROGRESS")
                || callInfo.state().name().equalsIgnoreCase("in-progress")) {
            if (mute) {
                if (!callInfo.isMuted()) {
