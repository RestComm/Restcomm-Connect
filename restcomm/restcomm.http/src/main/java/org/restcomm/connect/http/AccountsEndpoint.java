...
public class AccountsEndpoint extends SecuredEndpoint {
    ...
    /*
     * protected Response deleteAccount(final String operatedSid) { //First check if the account has the required permissions in
     * general, this way we can fail fast and avoid expensive DAO operations checkPermission("RestComm:Delete:Accounts"); //
     * what if effectiveAccount is null ?? - no need to check since we checkAuthenticatedAccount() in AccountsEndoint.init()
     * final Sid accountSid = userIdentityContext.getEffectiveAccount().getSid(); final Sid sidToBeRemoved = new
     * Sid(operatedSid);
     *
     * Account removedAccount = accountsDao.getAccount(sidToBeRemoved); secure(removedAccount, "RestComm:Delete:Accounts",
     * SecuredType.SECURED_ACCOUNT); // Prevent removal of Administrator account if
     * (operatedSid.equalsIgnoreCase(accountSid.toString())) return status(BAD_REQUEST).build();
     *
     * if (accountsDao.getAccount(sidToBeRemoved) == null) return status(NOT_FOUND).build();
     *
     * // the whole tree of sub-accounts has to be removed as well List<String> removedAccounts =
     * accountsDao.getSubAccountSidsRecursive(sidToBeRemoved); if (removedAccounts != null && !removedAccounts.isEmpty()) { int
     * i = removedAccounts.size(); // is is the count of accounts left to process while (i > 0) { i --; String removedSid =
     * removedAccounts.get(i); try { removeSingleAccount(removedSid); } catch (Exception e) { // if anything bad happens, log
     * the error and continue removing the rest of the accounts. logger.error("Failed removing (child) account '" + removedSid +
     * "'"); } } } // remove the parent account too removeSingleAccount(operatedSid);
     *
     * return ok().build(); }
     */
     ...
     *
     * // Remove its SIP client account clientDao.removeClients(sidToBeRemoved);
     *
     * return ok().build(); }
     */
    /**
    ...
    /**
     * Removes incoming phone numbers that belong to an account from the database. For provided numbers the provider is also
    ...
    protected Response putAccount(final MultivaluedMap<String, String> data, final MediaType responseType) {
        ...
        if (accountLineage.size() >= 2) {
            ...
            return status(BAD_REQUEST)
                    .entity(buildErrorResponseBody("This account is not allowed to have sub-accounts", responseType))
        ...
        try {
            ...
            return status(BAD_REQUEST).entity(buildErrorResponseBody("Password too weak", responseType)).type(responseType)
                    .build();
        }

        // If Account already exists don't add it again
        /*
         * Account creation rules: - either be Administrator or have the following permission: RestComm:Create:Accounts - only
         * Administrators can choose a role for newly created accounts. Normal users will create accounts with the same role as
         * their own.
         */
    ...
    private Client createClientFrom(final Sid accountSid, final MultivaluedMap<String, String> data) {
        ...
        final StringBuilder buffer = new StringBuilder();
        buffer.append("/").append(getApiVersion(data)).append("/Accounts/").append(accountSid.toString()).append("/Clients/")
                .append(sid.toString());
    ...
    private Account prepareAccountForUpdate(final Account account, final MultivaluedMap<String, String> data)
            throws AccountAlreadyClosed, PasswordTooWeak {
        ...
        try {
            ...
            if (newStatus != null) {
                ...
                // if this is a password reset operation we need to activate the account (in case there is no explicity Status
                // passed of course)
                if (isPasswordReset)
            ...
            if (data.containsKey("Role")) {
                ...
                // Only allow role change for administrators. Multitenancy checks will take care of restricting the modification
                // scope to sub-accounts.
    ...
    protected Response updateAccount(final String identifier, final MultivaluedMap<String, String> data,
        ...
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("At update account, exception trying to get SID. Seems we have email as identifier"); // TODO check
                                                                                                                   // when this
                                                                                                                   // exception
                                                                                                                   // is thrown
            }
    ...
    /**
     * Removes all resources belonging to an account and sets its status to CLOSED. If rcmlServerApi is not null it will also
     * send account-removal notifications to the rcmlserver
     *
     * @param closedAccount
     */
    ...
    /**
     * Closes an account along with all its children (the whole tree). Dependent entities are removed and, if configured,
     * notification are sent to the rcml server (RVD) as well.
     *
     * @param parentAccount
     */
