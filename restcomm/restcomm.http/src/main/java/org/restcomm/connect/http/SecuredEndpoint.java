...
/**
 * Security layer endpoint. It will scan the request for security related assets and populate the UserIdentityContext
 * accordingly. Extend the class and use checkAuthenticatedAccount*() methods to apply security rules to your endpoint.
 *
 * How to use it: - use checkAuthenticatedAccount() method to check that a user (any user) is authenticated. - use
 * checkAuthenticatedAccount(permission) method to check that an authenticated user has the required permission according to his
 * roles - use checkAuthenticatedAccount(account,permission) method to check that besides permission a user also has ownership
 * over an account
 *
...
public abstract class SecuredEndpoint extends AbstractEndpoint {
    ...
    /**
     * Grants access by permission. If the effective account has a role that resolves to the specified permission (accoording to
     * mappings of restcomm.xml) access is granted. Administrator is granted access regardless of permissions.
     *
    ...
    /**
     * Personalized type of grant. Besides checking 'permission' the effective account should have some sort of ownership over
     * the operatedAccount. The exact type of ownership is defined in secureAccount()
     *
    ...
    protected void secure(final Account operatedAccount, final String permission, SecuredType type)
            throws AuthorizationException {
    ...
    protected void secure(final Account operatedAccount, final Sid resourceAccountSid, SecuredType type)
            throws AuthorizationException {
        checkAuthenticatedAccount();
        if (operatedAccount == null) {
            // if operatedAccount is NULL, we'll probably return a 404. But let's handle that in a central place.
    ...
    // protected void secure(final Account operatedAccount, final Sid resourceAccountSid, final String permission) throws
    // AuthorizationException {
    // secure(operatedAccount, resourceAccountSid, permission, SecuredType.SECURED_STANDARD);
    // }
    //
    // protected void secure(final Account operatedAccount, final Sid resourceAccountSid, final String permission, final
    // SecuredType type ) {
    ...
    /**
     * Checks is the effective account has the specified role. Only role values contained in the Restcomm Account are take into
     * account.
     *
     * @param role
    ...
    /**
     * Low level permission checking. roleNames are checked for neededPermissionString permission using permission mappings
     * contained in restcomm.xml. The permission mappings are stored in RestcommRoles.
     *
     ...
     *
     * If no sub-resources are involved (resourceAccountSid is null): - If operatingAccount is the same or an ancestor of
     * operatedAccount, access is granted If there are sub-resources involved: - If operatingAccount is the same or an ancestor
     * of operatedAccount AND resource belongs to operatedAccount access is granted
     *
     ...
     *
     * If an application Account Sid is given: - If operatingAccount is the same as the operated account and application
     * resource belongs to operated account too acces is granted. If no application Account Sid is given: - If operatingAccount
     * is the same as the operated account access is granted.
     *
    ...
    private AuthOutcome secureLevelControlApplications(Account operatedAccount, String applicationAccountSid) {
         ...
         *
         * // operatingAccount and operatedAccount are not null at this point Account operatingAccount =
         * userIdentityContext.getEffectiveAccount(); String operatingAccountSid = operatingAccount.getSid().toString(); String
         * operatedAccountSid = operatedAccount.getSid().toString(); if
         * (!operatingAccountSid.equals(String.valueOf(operatedAccountSid))) { return AuthOutcome.FAILED; } else if
         * (applicationAccountSid != null && !operatingAccountSid.equals(applicationAccountSid)) { return AuthOutcome.FAILED; }
         * return AuthOutcome.OK;
         */
     ...
     * Applies the following access control rules:
     *
     * If the operating account is an administrator: - If it is the same or parent/ancestor of the operated account access is
     * granted. If the operating accoutn is NOT an administrator: - If it is the same as the operated account access is granted.
     *
     * @param operatedAccount
     * @return
     */
    ...
    /**
     * Returns the string literal for the administrator role. This role is granted implicitly access from
     * checkAuthenticatedAccount() method. No need to explicitly apply it at each protected resource .
     *
