...
public final class Downloader extends RestcommUntypedActor {
    ...
    public HttpResponseDescriptor fetch(final HttpRequestDescriptor descriptor)
        ...
        try {
            ...
            if (isHttpError(code)) {
                // TODO - usually this part of code is not reached. Error codes are part of error responses that do not pass
                // validateXML above and an exception is thrown. We need to re-thing this
            ...
            logger.warning(String.format("Problem while trying to download RCML. URL: %s, Status: %s, Response: %s ",
    ...
    public void onReceive(final Object message) throws Exception {
        ...
        if (HttpRequestDescriptor.class.equals(klass)) {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug("New HttpRequestDescriptor, method: " + request.getMethod() + " URI: " + request.getUri()
            ...
            if (sender != null && !sender.isTerminated()) {
                ...
                if (logger.isInfoEnabled()) {
                    logger.info("DownloaderResponse wont be send because sender is :"
                            + (sender.isTerminated() ? "terminated" : "null"));
                }
            }
        }
    }
    ...
    public HttpUriRequest request(final HttpRequestDescriptor descriptor)
        ...
        if ("GET".equalsIgnoreCase(method)) {
            ...
            if (query != null && !query.isEmpty()) {
                result = new URIBuilder().setScheme(uri.getScheme()).setHost(uri.getHost()).setPort(uri.getPort())
                        .setPath(uri.getPath()).setQuery(query).build();
            } else {
                result = uri;
            }
