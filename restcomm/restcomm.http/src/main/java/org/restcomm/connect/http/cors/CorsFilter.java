...
public class CorsFilter implements ContainerResponseFilter {
    ...
    public ContainerResponse filter(ContainerRequest cres, ContainerResponse response) {
        ...
        if (requestOrigin != null) { // is this is a cors request (ajax request that targets a different domain than the one the
                                     // page was loaded from)
            if (allowedOrigin != null && allowedOrigin.startsWith(requestOrigin)) { // no cors allowances make are applied if
                                                                                    // allowedOrigins == null
