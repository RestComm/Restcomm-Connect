...
public class BandwidthNumberProvisioningManager implements PhoneNumberProvisioningManager {
    ...
    public boolean cancelNumber(PhoneNumber phoneNumberObj) {
        ...
        try {
            ...
            if (response.getErrorList().size() == 0 && response.getorderRequest().getDisconnectTelephoneNumberOrderType()
                    .getTelephoneNumberList().get(0).equals(phoneNumber)) {
                isSucceeded = true;
            }
    ...
    private List<PhoneNumber> toPhoneNumbers(final SearchResult searchResult) {
        ...
        if (searchResult.getTelephoneNumberDetailList().size() > 0) {
            for (final TelephoneNumberDetail detail : searchResult.getTelephoneNumberDetailList()) {
                ...
                final PhoneNumber phoneNumber = new PhoneNumber(name, name, Integer.parseInt(detail.getLATA()),
                        detail.getRateCenter(), null, null, detail.getState(), null, "US", null, true, true, false, false,
                        false);
            ...
            for (final String number : searchResult.getTelephoneNumberList()) {
                ...
                final PhoneNumber phoneNumber = new PhoneNumber(name, name, null, null, null, null, null, null, "US", null,
