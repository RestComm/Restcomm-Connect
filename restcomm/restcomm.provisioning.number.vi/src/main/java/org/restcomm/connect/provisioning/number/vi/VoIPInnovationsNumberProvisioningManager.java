...
public class VoIPInnovationsNumberProvisioningManager implements PhoneNumberProvisioningManager {
    ...
    private List<PhoneNumber> toAvailablePhoneNumbers(final GetDIDListResponse response, PhoneNumberSearchFilters listFilters) {
        ...
        for (final State state : states) {
            if (listFilters.getInRegion() == null || (listFilters.getInRegion() != null && !listFilters.getInRegion().isEmpty()
                    && listFilters.getInRegion().equals(state.name()))) {
                            ...
                            for (final NXX nxx : npa.nxxs()) {
                                for (final TN tn : nxx.tns()) {
                                    ...
                                    if (searchPattern == null
                                            || (searchPattern != null && searchPattern.matcher(tn.number()).matches())) {
                                        if (listFilters.getFaxEnabled() == null || (listFilters.getFaxEnabled() != null
                                                && listFilters.getFaxEnabled() == tn.t38())) {
                                            ...
                                            final PhoneNumber number = new PhoneNumber(name, phoneNumber,
                                                    Integer.parseInt(lata.name()), center.name(), null, null, state.name(),
                                                    null, "US", null, true, null, null, tn.t38(), null);
                                            numbers.add(number);
                                        }
                                    }
                                }
        ...
        if (areaCode != null && !areaCode.isEmpty() && (areaCode.length() == 3)) {
            ...
            try {
                ...
                if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                    ...
                    logger.warn("Couldn't reach uri for getting DIDs. Response status was: "
