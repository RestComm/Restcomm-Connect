...
public final class Link extends RestcommUntypedActor {
    ...
    public Link(final ActorRef gateway, final MediaSession session, final NotifiedEntity agent, final long timeout,
            final ConnectionIdentifier overrideSecondaryConnId) {
        super();
        ...
        transitions.add(new Transition(initializingPrimary, closingPrimary));
    ...
    private final class InitializingPrimary extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            logger.info("Link: " + self().path() + " ,state: " + fsm.state() + " ,primaryEndpoint: " + primaryEndpoint.path()
                    + " ,secondaryEndpoint: " + secondaryEndpoint.path());
            if (primaryEndpoint != null && !primaryEndpoint.isTerminated()) {
                primaryEndpoint.tell(new InviteEndpoint(), source);
                logger.info("Link: " + self().path() + " ,state: " + fsm.state() + " InviteEndpoint sent to primaryEndpoint: "
                        + primaryEndpoint.path());
            } else {
                logger.info("Link: " + self().path() + " ,state: " + fsm.state()
                        + " InviteEndpoint DIDN'T sent to primaryEndpoint: " + primaryEndpoint.path()
                        + " primaryEndpoint is Terminated: " + primaryEndpoint.isTerminated());
            }
        }
    }
    ...
    private final class EnteringInitializingSecondary extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            logger.info("Link: " + self().path() + " ,state: " + fsm.state() + " ,primaryEndpointId: " + primaryEndpointId
                    + " ,secondaryEndpoint: " + secondaryEndpoint.path() + " secondaryEndpoint isTerminated: "
                    + secondaryEndpoint.isTerminated());
        }
    }
    ...
    private final class Opening extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            if (request.secondaryEndpointId() != null)
                secondaryEndpointId = new EndpointIdentifier(request.secondaryEndpointId(),
                        secondaryEndpointId.getDomainName());
