...
* Change the [source,bash]`<environments default="production">` to [source,bash]`<environments default="mariadb">`
...
* Start Mysql by running the command [source,bash]`sudo /etc/init.d/mysql start`
...
[source,bash]`mysql -u root -p < init.sql`
* Log into Mysql and make sure the Restcomm database was created by running the [source,bash]`show databases;` command.
