...
|UssdUrl |The URL Restcomm will request when receiving an incoming SMS message to this number.
|UssdMethod |The HTTP method Restcomm will use when making requests to the SmsUrl. Either GET or POST.
|UssdFallbackUrl |The URL that Restcomm will request if an error occurs retrieving or executing the RCML from SmsUrl.
|UssdFallbackMethod |The HTTP method Restcomm will use when requesting the above URL. Either GET or POST.
|UssdApplicationSid |The 34 character sid of the application Restcomm should use to handle SMSs sent to this number. If a SmsApplicationSid is present, Restcomm will ignore all of the SMS urls above and use those set on the application.
|UssdApplicationName |The application name.
|ReferUrl |The URL Restcomm will request when receiving an incoming SMS message to this number.
|ReferMethod |The HTTP method Restcomm will use when making requests to the SmsUrl. Either GET or POST.
|ReferApplicationSid |The 34 character sid of the application Restcomm should use to handle SMSs sent to this number. If a SmsApplicationSid is present, Restcomm will ignore all of the SMS urls above and use those set on the application.
