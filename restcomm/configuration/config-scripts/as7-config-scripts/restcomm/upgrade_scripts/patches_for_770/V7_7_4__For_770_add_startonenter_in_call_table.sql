...
CREATE PROCEDURE updateProcedure()
 BEGIN
 SELECT DISTINCTROW IFNULL(column_name, '') INTO @colName774
 FROM information_schema.columns
 WHERE table_schema='${RESTCOMM_DBNAME}'
 AND table_name = 'restcomm_call_detail_records'
 AND column_name = 'start_conference_on_enter';

IF @colName774 IS NULL THEN
CREATE TABLE temp_table LIKE restcomm_call_detail_records;
ALTER TABLE temp_table ADD start_conference_on_enter BOOLEAN;
