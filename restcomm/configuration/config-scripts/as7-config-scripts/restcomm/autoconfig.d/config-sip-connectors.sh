...
configConnectors() {
	...
	if [ "$ACTIVATE_LB" == "true" ] || [ "$ACTIVATE_LB" == "TRUE" ]; then
		...
		sed -e "s|path-name=\"org.mobicents.ext\" \(app-dispatcher-class=.*\)|path-name=\"org.mobicents.ha.balancing.only\" \1|" \
	        ...
	        -e "s|<connector name=\"sip-ws\" .*/>|<connector name=\"sip-ws\" protocol=\"SIP/2.0\" scheme=\"sip\" socket-binding=\"sip-ws\" use-static-address=\"true\" static-server-address=\"$LB_PUBLIC_IP\" static-server-port=\"$LB_EXTERNAL_PORT_WS\" use-load-balancer=\"true\" load-balancer-address=\"$LB_INTERNAL_IP\" load-balancer-rmi-port=\"$LB_RMI_PORT\"  load-balancer-sip-port=\"$LB_INTERNAL_PORT_WS\"/>|" \
