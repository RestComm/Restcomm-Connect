angular.module("rcApp.restcommApps").service("rappService", function ($http, $q, Notifications) {
	...
	function getAppConfig(appName, mode) {
		var defer = $q.defer();
		$http({url: '/restcomm-rvd/api/apps/' + appName + '/config' + (mode ? ("/"+mode) : "") , method: "GET" })
	...
	function getApp(appName) {
		var defer = $q.defer();
		$http({url: '/restcomm-rvd/api/apps/' + appName, method: "GET" })
		.success(function (data, status, headers, config) {
			if (data.rvdStatus == "OK") {
	...
	function getBoostrapObject(appName) {
		var deferred = $q.defer();
		$http.get('/restcomm-rvd/api/apps/' + appName + '/parameters' )
