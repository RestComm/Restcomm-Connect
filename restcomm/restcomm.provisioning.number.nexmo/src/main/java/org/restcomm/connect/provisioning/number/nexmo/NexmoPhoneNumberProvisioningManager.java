...
public class NexmoPhoneNumberProvisioningManager implements PhoneNumberProvisioningManager {
    ...
    private List<PhoneNumber> toAvailablePhoneNumbers(final JsonArray phoneNumbers, PhoneNumberSearchFilters listFilters) {
        ...
        for (int i = 0; i < phoneNumbers.size(); i++) {
            ...
            final PhoneNumber phoneNumber = new PhoneNumber(getFriendlyName(number.get("msisdn").getAsString(), countryCode),
                    number.get("msisdn").getAsString(), null, null, null, null, null, null, countryCode,
                    number.get("cost").getAsString(), isVoiceCapable, isSmsCapable, null, null, null);
        ...
        try {
            if ((phoneNumberParameters.getVoiceUrl() != null && !phoneNumberParameters.getVoiceUrl().isEmpty())
                ...
                if (phoneNumberParameters.getSmsUrl() != null && !phoneNumberParameters.getSmsUrl().isEmpty()
                    ...
                    updateUri = updateUri + "voiceCallbackValue="
                            + URLEncoder.encode(phoneNumber + "@" + phoneNumberParameters.getVoiceUrl(), "UTF-8")
                            + "&voiceCallbackType=sip" + "&moHttpUrl="
                            + URLEncoder.encode(phoneNumber + "@" + phoneNumberParameters.getSmsUrl(), "UTF-8")
                            + "&moSmppSysType=" + smppSystemType;
                } else if (phoneNumberParameters.getVoiceUrl() != null && !phoneNumberParameters.getVoiceUrl().isEmpty()) {
                    updateUri = updateUri + "voiceCallbackValue="
                            + URLEncoder.encode(phoneNumber + "@" + phoneNumberParameters.getVoiceUrl(), "UTF-8")
                            + "&voiceCallbackType=sip" + "&moSmppSysType=" + smppSystemType;
                } else {
                    updateUri = updateUri + "moHttpUrl="
                            + URLEncoder.encode(phoneNumber + "@" + phoneNumberParameters.getSmsUrl(), "UTF-8")
                            + "&moSmppSysType=" + smppSystemType;
                }
            }
            ...
            if (updateResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                if (logger.isDebugEnabled())
                    logger.debug("Couldn't update Phone Number " + phoneNumber + ". Response status was: "
    ...
    public boolean cancelNumber(PhoneNumber phoneNumberObj) {
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                if (logger.isDebugEnabled())
                    logger.debug("Couldn't cancel Phone Number " + phoneNumber + ". Response status was: "
                            + response.getStatusLine().getStatusCode());
            }
        } catch (final Exception e) {
