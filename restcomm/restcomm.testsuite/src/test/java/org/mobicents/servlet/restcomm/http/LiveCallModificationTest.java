...
public class LiveCallModificationTest {
	...
	@Test
	//Terminate a call in-progress using the Live Call Modification API. Non-regression test for issue: https://bitbucket.org/telestax/telscale-restcomm/issue/139
	public void terminateInProgressCall() throws Exception {
		...
		JsonObject callResult = RestcommCallsTool.getInstance().createCall(deploymentUrl.toString(), adminAccountSid, adminAuthToken, from, to, rcmlUrl);
		...
		assertTrue(bobCall.sendIncomingCallResponse(Response.OK, "OK-Bob", 3600, receivedBody, "application", "sdp", null, null));
		...
		callResult = RestcommCallsTool.getInstance().modifyCall(deploymentUrl.toString(), adminAccountSid , adminAuthToken, callSid, "completed", null);
	...
	@Test
	//Terminate a call that is ringing using the Live Call Modification API. Non-regression test for issue: https://bitbucket.org/telestax/telscale-restcomm/issue/139
	public void terminateRingingCall() throws Exception {
		...
		callResult = RestcommCallsTool.getInstance().modifyCall(deploymentUrl.toString(), adminAccountSid , adminAuthToken, callSid, "canceled", null);
	...
	public void redirectCall() throws Exception {
		...
		callResult = RestcommCallsTool.getInstance().modifyCall(deploymentUrl.toString(), adminAccountSid , adminAuthToken, callSid, null, rcmlUrl);
