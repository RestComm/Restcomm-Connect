...
public class MultitenancyDenyAccessApiTest {
    ...
    private enum Endpoint {
        ...
        INCOMING_PHONE_NUMBERS("IncomingPhoneNumbers", true, true, true, true, new HashMap<String, String>() {
            {
                put("PhoneNumber", "1111");
                put("AreaCode", "100");
            }
        }, "PNff22dc8d1cdf4d449d666ac09f0bb110", "PN9f9cf955aeb94ebb9d2e09cead5683a4"), CALLS("Calls", true, true, false, false,
                new HashMap<String, String>() {
                    {
                        put("From", "bob");
                        put("To", "alice");
                        put("Url", "/restcomm/url");
                    }
                }, "CA9aa1b61e9b864477a820d5c1c9d9bb7d", "CAc6a057e16aa74cb0923c538725ffcf01"), SMS_MESSAGES("SMS/Messages",
                        true, true, false, false, new HashMap<String, String>() {
                            {
                                put("From", "bob");
                                put("To", "alice");
                                put("Body", "Hi");
                            }
                        }, "SMa272937700b3461bb5d68a3569c61bf1", "SMa272937700b3461bb5d68a3569c61bf2"), CLIENTS("Clients", true,
                                true, true, true, new HashMap<String, String>() {
                                    {
                                        put("Login", "test");
                                        put("Password", "1234");
                                    }
                                }, "CLe95ba029114147c9a9aa42becd0518c0",
                                "CL9bfcb54ead2b44e6bae03f337967a249"), OUTGOING_CALLER_IDS("OutgoingCallerIds", true, true,
                                        false, true, new HashMap<String, String>() {
                                            {
                                                put("PhoneNumber", "1111");
                                            }
                                        }, "PNfa413fdbf3944932b37bef4bd661c7f7",
                                        "PN5a33fa8232d84578af023b1e81e30f67"), RECORDINGS("Recordings", true, false, false,
                                                false, null, "REacaffdf107da4dc3926e37bddfff44ed",
                                                "REacaffdf107da4dc3926e37bddfff44ee"), TRANSCRIPTIONS("Transcriptions", true,
                                                        false, false, true, null, "TRacaffdf107da4dc3926e37bddfff44ee",
                                                        "TRacaffdf107da4dc3926e37bddfff44ed"), NOTIFICATIONS("Notifications",
                                                                true, false, false, false, null,
                                                                "NO8927433ce9514b70ac0a76cd36601b9e",
                                                                "NO8927433ce9514b70ac0a76cd36601b9d");
        ...
        Endpoint(String name, boolean get, boolean postList, boolean postElement, boolean delete,
                HashMap<String, String> postParams, String elementSameAccount, String elementSubaccount) {
            this.name = name;
            this.get = get;
    ...
    public void deleteElementSameLevelAccount() throws ClientProtocolException, IOException {
        ...
        for (Endpoint endpoint : Endpoint.values()) {
            if (endpoint.delete) {
                ...
                assertTrue(statusCode == httpUnauthorized);
                logger.info(
                        "Tested endpoint " + endpoint.name + " (deleteElementSameLevelAccount). Status code = " + statusCode);
            }
        }
    }
    ...
    public void accountsApi() throws ClientProtocolException, IOException {
        ...
        int statusCode = RestcommMultitenancyTool.getInstance().get(baseUrl + jsonExtension + "/" + adminAccountSid,
                primaryUsername, accountsPassword);
        assertTrue(statusCode == httpUnauthorized);
        statusCode = RestcommMultitenancyTool.getInstance().post(baseUrl + jsonExtension + "/" + adminAccountSid,
                primaryUsername, accountsPassword, new HashMap<String, String>() {
                    {
                        put("EmailAddress", "test@test.com");
                        put("Password", "RestComm");
                    }
                });
        assertTrue(statusCode == httpUnauthorized);
        Map<String, String> updateParams = new HashMap<String, String>();
        updateParams.put("Status", "closed");
        statusCode = RestcommMultitenancyTool.getInstance().update(baseUrl + jsonExtension + "/" + adminAccountSid,
                primaryUsername, accountsPassword, updateParams);
        Assert.assertEquals(httpUnauthorized, statusCode);

        // Same level account
        statusCode = RestcommMultitenancyTool.getInstance().get(baseUrl + jsonExtension + "/" + secondaryAccountSid,
                primaryUsername, accountsPassword);
        assertTrue(statusCode == httpUnauthorized);
        statusCode = RestcommMultitenancyTool.getInstance().post(baseUrl + jsonExtension + "/" + secondaryAccountSid,
                primaryUsername, accountsPassword, new HashMap<String, String>() {
                    {
                        put("EmailAddress", "test@test.com");
                        put("Password", "RestComm");
        ...
        updateParams.put("Status", "closed");
        statusCode = RestcommMultitenancyTool.getInstance().update(baseUrl + jsonExtension + "/" + secondaryAccountSid,
                primaryUsername, accountsPassword, updateParams);
        assertTrue(statusCode == httpUnauthorized);
    }
    ...
    public void applicationsApi() throws ClientProtocolException, IOException {
        ...
        assertTrue(statusCode == httpUnauthorized);
        statusCode = RestcommMultitenancyTool.getInstance().post(baseUrl + jsonExtension, primaryUsername, accountsPassword,
                new HashMap<String, String>() {
                    {
                        put("FriendlyName", "TESTABC");
                    }
                });
        assertTrue(statusCode == httpUnauthorized);
        statusCode = RestcommMultitenancyTool.getInstance().post(baseUrl + "/" + adminApplicationSid + jsonExtension,
                primaryUsername, accountsPassword, new HashMap<String, String>() {
                    {
                        put("FriendlyName", "TESTABC");
                    }
        ...
        Map<String, String> updateParams = new HashMap<String, String>();
        updateParams.put("Status", "closed");
        statusCode = RestcommMultitenancyTool.getInstance().update(baseUrl + "/" + adminApplicationSid + jsonExtension,
                primaryUsername, accountsPassword, updateParams);
        assertTrue(statusCode == httpUnauthorized);
        ...
        assertTrue(statusCode == httpUnauthorized);
        statusCode = RestcommMultitenancyTool.getInstance().post(baseUrl + "/" + secondaryApplicationSid + jsonExtension,
                primaryUsername, accountsPassword, new HashMap<String, String>() {
                    {
                        put("FriendlyName", "TESTABC");
                    }
        ...
        updateParams = new HashMap<String, String>();
        updateParams.put("Status", "closed");
        statusCode = RestcommMultitenancyTool.getInstance().update(baseUrl + "/" + secondaryApplicationSid + jsonExtension,
                primaryUsername, accountsPassword, updateParams);
        assertTrue(statusCode == httpUnauthorized);
    }
