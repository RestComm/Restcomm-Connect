...
public class DialConferenceTest {
    ...
    private int getConferencesSize() {
        JsonObject conferences = RestcommConferenceTool.getInstance().getConferences(deploymentUrl.toString(), adminAccountSid,
                adminAuthToken);
    ...
    private int getParticipantsSize(final String name) {
        ...
        JsonObject participants = RestcommConferenceParticipantsTool.getInstance().getParticipants(deploymentUrl.toString(),
    ...
    private String dialConfernceRcmlWithTimeLimit = "<Response><Dial timeLimit=\"50\"><Conference>" + confRoom1
    ...
    public synchronized void testDialConferenceClientsWaitForDisconnect() throws InterruptedException {
        stubFor(get(urlPathEqualTo("/1111")).willReturn(
                aResponse().withStatus(200).withHeader("Content-Type", "text/xml").withBody(dialConfernceRcmlWithTimeLimit)));
        ...
        assertTrue(getParticipantsSize(confRoom1) == 2);
        ...
        Thread.sleep(1000);
        assertTrue(getConferencesSize() == 1);
        assertTrue(getParticipantsSize(confRoom1) == 0);
    ...
    public synchronized void testDialConferenceClientsDisconnect() throws InterruptedException {
        stubFor(get(urlPathEqualTo("/1111"))
                .willReturn(aResponse().withStatus(200).withHeader("Content-Type", "text/xml").withBody(dialConfernceRcml)));
    ...
    private String dialConfernceRcmlWithWaitUrl = "<Response><Dial><Conference startConferenceOnEnter=\"false\" waitUrl=\"http://127.0.0.1:8090/waitUrl\" waitMethod=\"GET\">"
            + confRoom3 + "</Conference></Dial></Response>";
    ...
    @Test
    @Ignore // TTS is not working on the testsuite
    public synchronized void testDialConferenceClientsDisconnectWithWaitUrl() throws InterruptedException {
        stubFor(get(urlPathEqualTo("/1111")).willReturn(
                aResponse().withStatus(200).withHeader("Content-Type", "text/xml").withBody(dialConfernceRcmlWithWaitUrl)));

        stubFor(get(urlPathEqualTo("/waitUrl"))
                .willReturn(aResponse().withStatus(200).withHeader("Content-Type", "text/xml").withBody(waitUrlRcml)));

        stubFor(get(urlPathEqualTo("/restcomm/audio/demo-prompt.wav"))
                .willReturn(aResponse().withStatus(200).withBodyFile("demo-prompt.wav")));
        ...
        Thread.sleep(1000);
        assertTrue(getConferencesSize() >= 1);
        assertTrue(getParticipantsSize(confRoom3) == 0);
        liveCalls = MonitoringServiceTool.getInstance().getStatistics(deploymentUrl.toString(), adminAccountSid,
                adminAuthToken);
        liveCallsArraySize = MonitoringServiceTool.getInstance().getLiveCallsArraySize(deploymentUrl.toString(),
                adminAccountSid, adminAuthToken);
    ...
    private String dialConfernceRcmlWithTimeLimit10Sec = "<Response><Dial timeLimit=\"10\"><Conference>" + confRoom4
            + "</Conference></Dial></Response>";
    ...
    public synchronized void testDialConferenceClientsDestroy() throws InterruptedException {
        ...
        int partSize = getParticipantsSize(confRoom4);
        ...
        partSize = getParticipantsSize(confRoom4);
        logger.info("Conference rooms: " + confSize + ", participants: " + partSize);
        assertTrue(confSize >= 1);
