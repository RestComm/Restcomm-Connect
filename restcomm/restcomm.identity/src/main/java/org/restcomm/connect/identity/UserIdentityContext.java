...
public class UserIdentityContext {
    ...
    final Account effectiveAccount; // if oauthToken is set get the account that maps to it. Otherwise use account from
                                    // accountKey
    Set<String> effectiveAccountRoles;
    List<String> accountLineage = null; // list of all parent account Sids up to the lop level account. It's initialized in a
                                        // lazy way.

    AccountsDao accountsDao;

    /**
     * After successfull creation of a UserIdentityContext object the following stands: - if an oauth token was present and
     * verified *oauthToken* will contain it. Otherwise it will be null - TODO If a *linked* account exists for the oauth token
     * username *effectiveAccount* will be set - if BASIC http credentials were present *accountKey* will contain them. Check
     * accountKey.isVerified() - if BASIC http credentials were verified effective account will be set to this account. - if
     * both oauthToken and accountKey are set and verified, effective account will be set to the account indicated by
     * accountKey.
     *
     * @param request
     * @param accountsDao
    ...
    /**
     * Returns the list of ancestors for the effective (the one specified in the credentials) account in a lazy way.
     *
     * @return
     */
            ...
            if (effectiveAccount != null) {
                try {
                    ...
                    throw new RuntimeException(
                            "Logged account has a very big line of ancestors. Something seems wrong. Account sid: "
                                    + effectiveAccount.getSid().toString(),
                            e);
                }
            }
        }
