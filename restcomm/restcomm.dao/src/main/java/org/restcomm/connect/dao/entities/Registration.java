...
public final class Registration implements Comparable<Registration> {
    ...
    public Registration(final Sid sid, final String instanceId, final DateTime dateCreated, final DateTime dateUpdated,
        ...
        this(sid, instanceId, dateCreated, dateUpdated, DateTime.now().plusSeconds(timeToLive), addressOfRecord, displayName,
                userName, userAgent, timeToLive, location, webRTC, isLBPresent);
    }

    public Registration(final Sid sid, final String instanceId, final DateTime dateCreated, final DateTime dateUpdated,
            final DateTime dateExpires, final String addressOfRecord, final String displayName, final String userName,
            final String userAgent, final int timeToLive, final String location, final boolean webRTC,
            final boolean isLBPresent) {
        super();
        this.sid = sid;
    ...
    public Registration setTimeToLive(final int timeToLive) {
        ...
        return new Registration(sid, instanceId, dateCreated, now, now.plusSeconds(timeToLive), addressOfRecord, displayName,
                userName, userAgent, timeToLive, location, webrtc, isLBPresent);
    }

    public Registration updated() {
        final DateTime now = DateTime.now();
        return new Registration(sid, instanceId, dateCreated, now, dateExpires, addressOfRecord, displayName, userName,
