...
public final class MmsConferenceController extends MediaServerController {
    ...
    public MmsConferenceController(final ActorRef mrb) {
        ...
        this.acquiringCnfMediaResourceController = new State("acquiring Cnf Media Resource Controller",
                new AcquiringCnfMediaResourceController(source), null);
    ...
    public void onReceive(Object message) throws Exception {
        ...
        if (Observe.class.equals(klass)) {
           ...
           * else if (MediaGroupStateChanged.class.equals(klass)) { onMediaGroupStateChanged((MediaGroupStateChanged) message,
           * self, sender); }
           */ else if (JoinCall.class.equals(klass)) {
            ...
            /*
             * to media-server as media-server will automatically stop beep when it will receive play command for beep. If a
             * beep wont be played, then conference need to send EndSignal(StopMediaGroup) to media-server to stop ongoing
             * music-on-hold. https://github.com/RestComm/Restcomm-Connect/issues/2024
             */
            if (!msg.beep()) {
                conferenceMediaResourceController.tell(message, sender);
            }
            ...
            onConferenceMediaResourceControllerStateChanged((ConferenceMediaResourceControllerStateChanged) message, self,
                    sender);
        }
    }
    ...
    private void onMediaResourceBrokerResponse(MediaResourceBrokerResponse<?> message, ActorRef self, ActorRef sender)
            throws Exception {
        if (logger.isInfoEnabled())
        ...
        if (is(acquiringMediaGateway)) {
            ...
            // for master's first join it would be null but if master left and rejoin it should join on same ep as received by
            // mrb.
            ...
            conferenceMediaResourceController
                    .tell(new StartConferenceMediaResourceController(this.cnfEndpoint, this.conferenceSid), self);
        }
    }

    private void onConferenceMediaResourceControllerStateChanged(ConferenceMediaResourceControllerStateChanged message,
            ActorRef self, ActorRef sender) throws Exception {
        if (logger.isDebugEnabled())
     ...
     * switch (message.state()) { case ACTIVE: if (is(creatingMediaGroup)) { fsm.transition(message,
     * gettingCnfMediaResourceController); } break;
     *
     * case INACTIVE: if (is(creatingMediaGroup)) { this.fail = Boolean.TRUE; fsm.transition(message, failed); } else if
     * (is(stopping)) { // Stop media group actor this.mediaGroup.tell(new StopObserving(self), self);
     * context().stop(mediaGroup); this.mediaGroup = null;
     *
     * // Move to next state if (this.mediaGroup == null && this.cnfEndpoint == null) { this.fsm.transition(message, fail ?
    ...
    /*
     * private void onStopMediaGroup(StopMediaGroup message, ActorRef self, ActorRef sender) { if (is(active)) { // Stop the
     * primary media group this.mediaGroup.tell(new Stop(), self); this.playing = Boolean.FALSE; } }
     *
     * private void onPlay(Play message, ActorRef self, ActorRef sender) { if (is(active) && !playing) { if
     * (logger.isInfoEnabled()) { logger.info("Received Play message, isActive: "+is(active)+" , is playing: "+playing); }
     * this.playing = Boolean.TRUE; this.mediaGroup.tell(message, self); } else { if (logger.isInfoEnabled()) {
     * logger.info("Received Play message but wont process it, isActive: "+is(active)+" , is playing: "+playing); } } }
     *
     * private void onStartRecording(StartRecording message, ActorRef self, ActorRef sender) throws Exception { if (is(active)
     * && !recording) { String finishOnKey = "1234567890*#"; int maxLength = 3600; int timeout = 5;
     *
     * this.recording = Boolean.TRUE; this.recordStarted = DateTime.now();
     *
     * // Tell media group to start recording Record record = new Record(message.getRecordingUri(), timeout, maxLength,
     * finishOnKey); this.mediaGroup.tell(record, null); } }
     *
     * private void onStopRecording(StopRecording message, ActorRef self, ActorRef sender) throws Exception { if (is(active) &&
     * recording) { this.recording = Boolean.FALSE; mediaGroup.tell(new Stop(), null); } }
     *
     * private void onMediaGroupResponse(MediaGroupResponse<String> message, ActorRef self, ActorRef sender) throws Exception {
     * if (is(active) && this.playing) { this.playing = Boolean.FALSE; } }
     */
    ...
    private void onEndpointStateChanged(EndpointStateChanged message, ActorRef self, ActorRef sender) throws Exception {
        if (is(stopping)) {
            if (sender.equals(this.cnfEndpoint) && (EndpointState.DESTROYED.equals(message.getState())
                    || EndpointState.FAILED.equals(message.getState()))) {
                if (EndpointState.FAILED.equals(message.getState()))
                    logger.error("Could not destroy endpoint on media server. corresponding actor path is: "
                            + this.cnfEndpoint.path());
    ...
    private final class AcquiringCnfMediaResourceController extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            if (logger.isInfoEnabled())
                logger.info("MMSConferenceController: GettingCnfMediaResourceController: conferenceName = " + conferenceName
                        + " conferenceSid: " + conferenceSid + " cnfenpointID: " + cnfEndpoint);
    ...
    private final class Active extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            broadcast(new MediaServerConferenceControllerStateChanged(MediaServerControllerState.ACTIVE, conferenceSid,
                    msg.conferenceState(), msg.moderatorPresent()));
        }
    }
    ...
    private final class Stopping extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            // CMRC might ask you not to destroy endpoint bcz master have left firt and other slaves are still connected to this
            // conference endpoint.
            if (response.destroyEndpoint()) {
                ...
                if (logger.isInfoEnabled())
                    logger.info(
                            "CMRC have ask you not to destroy endpoint bcz master have left firt and other slaves are still connected to this conference endpoint");
