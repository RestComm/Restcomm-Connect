            ...
            if (number != null) {
                ...
                if (appUri != null) {
                    ...
                    if (sid != null) {
                        ...
                        builder.setUrl(UriUtils.resolve(servletContext, request.getLocalAddr(), application.getSmsUrl()));
                        builder.setMethod(application.getSmsMethod());
                        builder.setFallbackUrl(UriUtils.resolve(servletContext, request.getLocalAddr(), application.getSmsFallbackUrl()));
                        builder.setFallbackMethod(application.getSmsFallbackMethod());
                    } else {
                        builder.setUrl(UriUtils.resolve(servletContext, request.getLocalAddr(), appUri));
                        builder.setMethod(number.getSmsMethod());
                        URI appFallbackUrl = number.getSmsFallbackUrl();
                        if (appFallbackUrl != null) {
                            builder.setFallbackUrl(UriUtils.resolve(servletContext, request.getLocalAddr(), number.getSmsFallbackUrl()));
