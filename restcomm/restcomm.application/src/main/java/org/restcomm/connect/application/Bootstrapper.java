...
public final class Bootstrapper extends SipServlet implements SipServletListener {
    ...
    private MediaServerControllerFactory mediaServerControllerFactory(final Configuration configuration, ClassLoader loader,
            DaoManager storage) throws ServletException {
    ...
    private SipURI outboundInterface(ServletContext context, String transport) {
        ...
        if (uris != null && uris.size() > 0) {
            ...
            if (logger.isInfoEnabled()) {
                if (result == null) {
                    logger.info(
                            "Outbound interface is NULL! Looks like there was no " + transport + " in the list of connectors");
                } else {
    ...
    private ActorRef mediaResourceBroker(final Configuration configuration, final DaoManager storage, final ClassLoader loader)
    ...
    private DaoManager storage(final Configuration configuration, Configuration daoManagerConfiguration,
    ...
    private ActorRef monitoringService(final Configuration configuration, final DaoManager daoManager,
    ...
    public void servletInitialized(SipServletContextEvent event) {
        if (event.getSipServlet().getClass().equals(Bootstrapper.class)) {
            ...
            Boolean rvdMigrationEnabled = new Boolean(
                    xml.subset("runtime-settings").getString("rvd-workspace-migration-enabled", "false"));
