 ...
 * {@link Sid.Type.PROJECT} generated to each entry.
 *
...
public class RvdProjectsMigrator {
    ...
    public void executeMigration() throws Exception {
        ...
        try {
            ...
            try {
                ...
                storeNewMessage("[ERROR-CODE:2] Error while storing workspace status" + x.getMessage(), true, true, false,
                        true);
            }
            throw e;
        }
    ...
    private String synchronizeApplicationEntity(String applicationSid, String projectName)
        ...
        if (!projectName.equals(applicationSid)) {
            ...
            storeNewMessage("Project '" + projectName + "' synchronized with Application '" + applicationSid + "'", false, true,
                    false, false);
        } else {
            storeNewMessage(
                    "Project '" + projectName + "' previously synchronized with Application '" + applicationSid + "'. Skipped",
                    false, true, false, false);
        }
        return applicationSid;
    }

    private void migrateNamingConvention(String projectName, String applicationSid)
            throws RvdProjectsMigrationException, URISyntaxException {
        if (!projectName.equals(applicationSid)) {
    ...
    private void updateIncomingPhoneNumbers(String applicationSid, String projectName)
            throws RvdProjectsMigrationException, URISyntaxException {
        ...
        if (amountUpdated > 0) {
            ...
            storeNewMessage("No IncomingPhoneNumbers found to update with Application '" + applicationSid + "'. Skipped", false,
                    true, false, false);
        }
    }

    private void updateClients(String applicationSid, String projectName)
            throws RvdProjectsMigrationException, URISyntaxException {
        ...
        if (amountUpdated > 0) {
            ...
            storeNewMessage("No Clients found to update with Application '" + applicationSid + "'. Skipped", false, true, false,
                    false);
        }
    }
