...
/**
 * This class was designed to be used with exclusivity by {@link RvdProjectsMigrator}, once that Restcomm should not interact
 * with RVD's workspace as a typical operation given that the migration process forms a very specific scenario.
 *
...
public class RvdProjectsMigrationHelper {
    ...
    public List<String> listProjects() throws RvdProjectsMigrationException {
        ...
        if (workspaceDir.exists()) {
            ...
            throw new RvdProjectsMigrationException("[ERROR-CODE:1] Error while loading the list of projects from workspace",
                    1);
        }
        return items;
    }
    ...
    public String searchApplicationSid(String projectName) throws RvdProjectsMigrationException {
        try {
            ...
            /*
             * This happens when a non-upgraded project whose friendly-name exists in several applications is upgraded. The old
             * bahaviour was a broken upgrade attempt for the whole workspace. Now, the failure should be limited to this
             * project
             */
            throw new RvdProjectsMigrationException(
                    "[ERROR-CODE:14] Error while upgrading project '" + projectName
                            + "'. Several applications with such a FriendlyName were found. You have to manually restore this project.",
                    14);
        }
    }

    public void renameProjectUsingNewConvention(String projectName, String applicationSid)
            throws RvdProjectsMigrationException {
        try {
            renameProject(projectName, applicationSid);
        } catch (IOException e) {
            throw new RvdProjectsMigrationException("[ERROR-CODE:5] Error while renaming the project '" + projectName + "' to '"
                    + applicationSid + "'" + e.getMessage(), 5);
        }
    }
    ...
    public void loadProjectState(String projectName) throws RvdProjectsMigrationException {
        try {
            ...
            throw new RvdProjectsMigrationException(
                    "[ERROR-CODE:6] Error loading state file from project '" + projectName + "' " + e.getMessage(), 6);
        }
    }
        ...
        try {
            if (applicationSid != null) {
                ...
                currentApplication = currentApplication
                        .setRcmlUrl(URI.create("/restcomm-rvd/services/apps/" + applicationSid + "/controller"));
                ...
                if (account == null) {
                    throw new RvdProjectsMigrationException(
                            "Error locating the owner account for project \"" + projectName + "\"");
                }
            ...
            String suffix = currentApplication != null
                    ? "with the application '" + currentApplication.getSid().toString() + "' " : "";
            throw new RvdProjectsMigrationException(
                    "[ERROR-CODE:7] Error while synchronizing the project '" + projectName + "' " + suffix + e.getMessage(), 7);
        }
    }
    ...
    public int updateIncomingPhoneNumbers(String applicationSid, String projectName) throws RvdProjectsMigrationException {
        ...
        try {
                    ...
                    for (int i = 0; i < dids.size(); i++) {
                        ...
                        if (hasUrlReference(did.getSmsUrl(), currentApplication.getFriendlyName())) {
                            ...
                            IncomingPhoneNumber updateSmsDid = new IncomingPhoneNumber(did.getSid(), did.getDateCreated(),
                                    ...
                                    did.getUssdFallbackMethod(), did.getUssdApplicationSid(), did.getReferUrl(),
                                    did.getReferMethod(), did.getReferApplicationSid(), did.isVoiceCapable(),
                    ...
                    for (int i = 0; i < dids.size(); i++) {
                        ...
                        if (hasUrlReference(did.getUssdUrl(), currentApplication.getFriendlyName())) {
                            ...
                            IncomingPhoneNumber updateUssdDid = new IncomingPhoneNumber(did.getSid(), did.getDateCreated(),
                                    ...
                                    did.getReferUrl(), did.getReferMethod(), did.getReferApplicationSid(), did.isVoiceCapable(),
                                    did.isSmsCapable(), did.isMmsCapable(), did.isFaxCapable(), did.isPureSip());
            ...
            throw new RvdProjectsMigrationException("[ERROR-CODE:9] Error while updating IncomingPhoneNumber '"
                    + did.getSid().toString() + "' with the Application '" + applicationSid + "' " + e.getMessage(), 9);
        }
    ...
    public int updateClients(String applicationSid, String projectName) throws RvdProjectsMigrationException {
        ...
        try {
            ...
            throw new RvdProjectsMigrationException("[ERROR-CODE:11] Error while updating Client '" + client.getSid().toString()
                    + "' with the Application '" + applicationSid + "' " + e.getMessage(), 11);
        }
        return amountUpdated;
    }
    ...
    public boolean isMigrationEnabled() {
        Boolean value = new Boolean(
                configuration.subset("runtime-settings").getString("rvd-workspace-migration-enabled", "true"));
        return value;
    }
