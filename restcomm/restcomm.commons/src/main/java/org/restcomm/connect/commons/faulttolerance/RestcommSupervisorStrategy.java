...
public class RestcommSupervisorStrategy implements SupervisorStrategyConfigurator {
    ...
    private static class RestcommFaultToleranceStrategy extends OneForOneStrategy {
        ...
        public RestcommFaultToleranceStrategy(int maxNrOfRetries, Duration withinTimeRange,
                Function<Throwable, Directive> function) {
            super(maxNrOfRetries, withinTimeRange, function);
        }

        @Override
        public boolean handleFailure(ActorContext context, ActorRef child, Throwable cause, ChildRestartStats stats,
                Iterable<ChildRestartStats> children) {
            String msg = String.format(
                    "RestcommSupervisorStrategy, actor exception handling. Actor path %s, exception cause %s, default exception handling strategy %s",
                    child.path().toString(), cause, strategy.toString());
            logger.error(msg);
        ...
        // public void processFailure(ActorContext context, boolean restart, ActorRef child, Throwable cause, ChildRestartStats
        // stats, Iterable<ChildRestartStats> children) {
        // String msg = String.format("RestcommSupervisor, actor exception handling. Restart %s, actor path %s, cause %s,",
        // restart, child.path().toString(), cause);
        // logger.error(msg);
    ...
    private static class RestcommFaultToleranceDecider implements Function<Throwable, SupervisorStrategy.Directive> {
        ...
        public SupervisorStrategy.Directive apply(Throwable t) throws Exception {
            // String msg = String.format("Handling exception %s with default strategy to %s", t.getClass().getName(),
            // strategy.toString());
            // logger.error(msg);
            return strategy;
            // return resume();
