    ...
    public URI download(URI requestUri, File pathToSave) throws IOException, URISyntaxException {
        ...
        try {
            if (requestUri.getScheme().equalsIgnoreCase("https")) {
                ...
                /*
                 * URI result = new URIBuilder() .setScheme(requestUri.getScheme()) .setHost(requestUri.getHost())
                 * .setPort(requestUri.getPort()) .setPath(requestUri.getPath()) .build(); HttpGet httpRequest = new
                 * HttpGet(result);
                 */
