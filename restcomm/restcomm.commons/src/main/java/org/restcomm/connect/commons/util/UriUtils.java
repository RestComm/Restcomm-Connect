...
public final class UriUtils {
    ...
    private static HttpConnectorList getHttpConnectors() throws MalformedObjectNameException, NullPointerException,
            UnknownHostException, AttributeNotFoundException, InstanceNotFoundException, MBeanException, ReflectionException {
        MBeanServer mbs = ManagementFactory.getPlatformMBeanServer();

        Set<ObjectName> jbossObjs = mbs
                .queryNames(new ObjectName("jboss.as:socket-binding-group=standard-sockets,socket-binding=http*"), null);
        Set<ObjectName> tomcatObjs = mbs.queryNames(new ObjectName("*:type=Connector,*"),
        ...
        if (jbossObjs != null && jbossObjs.size() > 0) {
            ...
            for (Iterator<ObjectName> i = tomcatObjs.iterator(); i.hasNext();) {
                ...
                HttpConnector httpConnector = new HttpConnector(scheme, address, Integer.parseInt(port),
