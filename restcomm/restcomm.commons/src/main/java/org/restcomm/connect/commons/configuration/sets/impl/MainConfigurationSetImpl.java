...
/**
 * Provides a typed interface to a set of configuration options retrieved from a configuration source.
 *
 * To add a new option in this set define its name as static fields and then initialize, validate it in the constructor.
 *
...
public class MainConfigurationSetImpl extends ConfigurationSet implements MainConfigurationSet {
    ...
    public MainConfigurationSetImpl(ConfigurationSource source) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "Error initializing '" + USE_HOSTNAME_TO_RESOLVE_RELATIVE_URL_KEY + "' configuration setting", e);
        }
    ...
    public MainConfigurationSetImpl(SslMode sslMode, int responseTimeout, boolean useHostnameToResolveRelativeUrls,
            String hostname, String instanceId, boolean bypassLbForClients) {
        super(null);
        this.sslMode = sslMode;
