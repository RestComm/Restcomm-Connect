...
public class CallControlHelper {
	...
	static public boolean permitted(final String authorization, final String method, DaoManager daoManager) {
	...
	/**
	 * 
	 * Check if a client is authenticated. If so, return true.
	 * Otherwise request authentication and return false;
	 * 
	 * @return
	 * @throws IOException 
	 */
	static public boolean checkAuthentication(SipServletRequest request, DaoManager storage) throws IOException {
	...
	static public void authenticate(final SipServletRequest request) throws IOException {
	...
	static private String nonce() {
	    final byte[] uuid = UUID.randomUUID().toString().getBytes();
	    final char[] hex = toHex(uuid);
		return new String(hex).substring(0, 31);
	  }

	static private String header(final String nonce, final String realm, final String scheme) {
