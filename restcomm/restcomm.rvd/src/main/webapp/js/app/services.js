...
angular.module('Rvd').service('projectSettingsService', ['$http','$q','$modal', '$resource', function ($http,$q,$modal,$resource) {
	...
	service.refresh = function (projectSid) {
		var resource = $resource('services/projects/:projectSid/settings');
		cachedProjectSettings = resource.get({projectSid:projectSid});
	}
	
	service.retrieve = function (projectSid) {
		var deferred = $q.defer();
		$http({method:'GET', url:'services/projects/'+projectSid+'/settings'})
	...
	service.save = function (projectSid, projectSettings) {
		var deferred = $q.defer();
		$http({method:'POST',url:'services/projects/'+projectSid+'/settings',data:projectSettings})
		.success(function (data,status) {deferred.resolve()})
		.error(function (data,status) {deferred.reject('ERROR_SAVING_PROJECT_SETTINGS')});
		return deferred.promise;
	}
	
	function projectSettingsModelCtrl ($scope, projectSettings, projectSettingsService, projectSid, projectName, $modalInstance, notifications) {
		//console.log("in projectSettingsModelCtrl");
		$scope.projectSettings = projectSettings;
		$scope.projectName = projectName;
		$scope.projectSid = projectSid;
		
		$scope.save = function (projectSid, data) {
			//console.log("saving projectSettings for " + name);
			service.save(projectSid, data).then(
	...
	service.showModal = function(projectSid, projectName) {
		var modalInstance = $modal.open({
			  ...
			  resolve: {
				projectSettings: function () {
					var deferred = $q.defer()
					$http.get("services/projects/"+projectSid+"/settings")
				...
				projectSid: function () {return projectSid;}
			  }
			});

			modalInstance.result.then(function (projectSettings) {
				service.refresh(projectSid);
...
angular.module('Rvd').service('webTriggerService', ['$http','$q','$modal', function ($http,$q,$modal) {
	...
	service.retrieve = function (projectSid) {
		var deferred = $q.defer();
		$http({method:'GET', url:'services/projects/'+projectSid+'/cc'})
	...
	service.save = function (projectSid, ccInfo) {
		var deferred = $q.defer();
		$http({method:'POST',url:'services/projects/'+projectSid+'/cc',data:ccInfo})
		.success(function (data,status) {deferred.resolve()})
		.error(function (data,status) {deferred.reject('ERROR_SAVING_PROJECT_CC')});
		return deferred.promise;
	}
	
	function webTriggerModalCtrl ($scope, ccInfo, projectSid, rvdSettings, $modalInstance, notifications, $location) {
		...
		$scope.getWebTriggerUrl = function () {
			return $location.protocol() + "://" + $location.host() + ":" +  $location.port() + "/restcomm-rvd/services/apps/" +  projectSid + '/start<span class="text-muted">?from=12345&amp;to=+1231231231&amp;token=mysecret</span>';
		};
	...
	service.showModal = function(projectSid) {
		var modalInstance = $modal.open({
			  ...
			  resolve: {
				ccInfo: function () {
					var deferred = $q.defer()
					$http.get("services/projects/"+projectSid+"/cc")
					.then(function (response) {
						deferred.resolve(response.data);
					}, function (response) {
						if ( response.status == 404 )
...
angular.module('Rvd').service('projectLogService', ['$http','$q','$routeParams', 'notifications', function ($http,$q,$routeParams,notifications) {
	...
	service.retrieve = function () {
		var deferred = $q.defer();
		$http({method:'GET', url:'services/apps/'+$routeParams.projectSid+'/log'})
		.success(function (data,status) {
	...
	service.reset = function () {
		var deferred = $q.defer();
		$http({method:'DELETE', url:'services/apps/'+$routeParams.projectSid+'/log'})
		.success(function (data,status) {
