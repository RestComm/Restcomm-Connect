...
angular.module('Rvd').controller('wavManagerController', function ($rootScope, $scope, $http, $upload) {
	$scope.deleteWav = function (wavItem) {
		$http({url: 'services/projects/' + $scope.projectSid + '/wavs?filename=' + wavItem.filename, method: "DELETE"})
	...
	$scope.onFileSelect = function($files) {
		    ...
		    for (var i = 0; i < $files.length; i++) {
		      ...
		      $scope.upload = $upload.upload({
		        url: 'services/projects/' + $scope.projectSid + '/wavs',
		        file: file,
		      }).success(function(data, status, headers, config) {
