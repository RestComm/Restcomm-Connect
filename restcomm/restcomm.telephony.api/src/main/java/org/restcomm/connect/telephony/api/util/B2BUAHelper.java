...
public class B2BUAHelper {
        ...
        try {
            ...
            if (initialIpBeforeLB != null) {
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("We are behind load balancer, initial IP and Ports are  " + initialIpBeforeLB + ":"
                            + initialPortBeforeLB);
                }
                isBehindLB = true;
            }
    ...
    public static void forwardResponse(final SipServletResponse response, final boolean patchForNat) throws IOException {
        ...
        if (response.getSession() != null && (!response.getSession().getState().equals(SipSession.State.TERMINATED))) {
