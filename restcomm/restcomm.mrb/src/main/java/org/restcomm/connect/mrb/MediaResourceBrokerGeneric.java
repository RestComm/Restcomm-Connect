...
public class MediaResourceBrokerGeneric extends RestcommUntypedActor {
     ...
     */
    protected void onStartMediaResourceBroker(StartMediaResourceBroker message, ActorRef self, ActorRef sender)
            throws UnknownHostException {
    ...
    protected ActorRef turnOnMediaGateway(MediaServerEntity mediaServerEntity) throws UnknownHostException {
        ...
        if (logger.isInfoEnabled())
            logger.info("turnOnMediaGateway local ip: " + localMediaServerEntity.getLocalIpAddress() + " local port: "
                    + localMediaServerEntity.getLocalPort() + " remote ip: " + mediaServerEntity.getRemoteIpAddress()
                    + " remote port: " + mediaServerEntity.getRemotePort());
    ...
    /**
     * @param message - GetMediaGateway - (callSid: if MediaGateway is required for a call - conferenceName: if MediaGateway is
     *        required for a conference - msId: or if MediaGateway is required for a specific MediaServer in cluster)
     * @param self
     * @param sender
     * @throws Exception
     */
     ...
     * @throws Exception
     */
    protected MediaGatewayForConference addConferenceDetailRecord(final String conferenceName, final Sid callSid)
            throws Exception {
        Sid sid = null;
        ...
        if (conferenceName == null) {
            ...
            if (callRecord != null) {
                ...
                ConferenceDetailRecordFilter filter = new ConferenceDetailRecordFilter(accountSid, "RUNNING%", null, null,
                        friendlyName, 1, 0);
                ...
                if (records != null && records.size() > 0) {
                    ...
                    if (logger.isInfoEnabled())
                        logger.info(
                                "A conference with same name is running. According to database record. given SID is: " + sid);
                } else {
                    // this is first record of this conference on all instances of
                    addNewConferenceRecord(accountSid, callRecord, friendlyName);

                    // getting CDR again as it is a conditional insert(select if exists or insert) to handle concurrency (incase
                    // another participant joins on another instance at very same time)
    ...
    protected void addNewConferenceRecord(String accountSid, CallDetailRecord callRecord, String friendlyName) {
        ...
        UriBuffer.append("/").append(callRecord.getApiVersion()).append("/Accounts/").append(accountSid)
                .append("/Conferences/");
