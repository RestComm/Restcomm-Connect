...
public class ConferenceMediaResourceControllerGeneric extends RestcommUntypedActor {
    ...
    public ConferenceMediaResourceControllerGeneric(ActorRef localMediaGateway, final Configuration configuration,
            final DaoManager storage, final ActorRef mrb) {
        super();
    ...
    public void onReceive(Object message) throws Exception {
        ...
        if (logger.isInfoEnabled()) {
            logger.info(" ********** ConferenceMediaResourceController " + self().path() + " Processing Message: "
                    + klass.getName());
            logger.info(
                    " ********** ConferenceMediaResourceController " + self().path() + " Current State: \"" + state.toString());
        }
        ...
        try {
            ...
            try {
                ...
                /*
                 * some state might not allow direct transition to failed state: in that case catch the
                 * TransitionFailedException and print error.
                 */
                logger.error("CMRC failed at a state which does not allow direct transition to FAILED state. Current state is: "
                        + state.toString());
            }
        }
    }
    ...
    protected void onStartConferenceMediaResourceController(StartConferenceMediaResourceController message, ActorRef self,
            ActorRef sender) throws Exception {
        if (is(uninitialized)) {
            if (logger.isDebugEnabled())
                logger.debug("onStartConferenceMediaResourceController: conferenceSid: " + message.conferenceSid()
                        + " cnfEndpoint: " + message.cnfEndpoint());
    ...
    protected void onMediaGroupStateChanged(MediaGroupStateChanged message, ActorRef self, ActorRef sender) throws Exception {
        if (logger.isDebugEnabled())
            logger.debug("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ onMediaGroupStateChanged - received STATE is: "
                    + message.state() + " current fsm STATE is: " + fsm.state()
                    + " ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
    ...
    protected void onPlay(Play message, ActorRef self, ActorRef sender) {
        /*
         * https://github.com/RestComm/Restcomm-Connect/issues/2024 We actually dont care about running beeps or MOH, we will
         * send new play it will stop exiting playing audio and will play new one (unless both are exactly same)
         */
        // if (!playing) {
    ...
    protected final class PreActive extends AbstractAction {
        ...
        public void execute(final Object message) throws Exception {
            ...
            broadcast(new ConferenceMediaResourceControllerStateChanged(
                    ConferenceMediaResourceControllerStateChanged.MediaServerControllerState.ACTIVE, cdr.getStatus()));
    ...
    protected abstract class FinalState extends AbstractAction {
        ...
        public FinalState(ActorRef source,
                final ConferenceMediaResourceControllerStateChanged.MediaServerControllerState state) {
            super(source);
            this.state = state;
        }
