...
sleep 3
$SIPP_EXECUTABLE -sf $CURRENT_FOLDER/tests/gather/gather-sipp.xml -s 1236 $RESTCOMM_ADDRESS:5080 -p 5090 -i $LOCAL_ADDRESS -mi $LOCAL_ADDRESS -l $SIMULTANEOUS_CALLS -m $MAXIMUM_CALLS -aa -rate_increase 10 -rate_interval 10s -rate_max $CALL_RATE -no_rate_quit -recv_timeout 10000 -t un -nr -fd 1 -trace_stat -stf $RESULTS_DIR/gather-$DATE.csv -trace_screen -screen_file $RESULTS_DIR/gather-$DATE-screens.log -trace_rtt -trace_err -error_file $RESULTS_DIR/gather-$DATE-errors.log
