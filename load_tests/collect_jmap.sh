...
if [[ -z "$RESTCOMM_PID" ]]; then
    ...
    echo "****************************************************************" > $BASEDIR/restcomm_mem
    echo "GC Histogram before GC.run" >> $BASEDIR/restcomm_mem
    echo "****************************************************************" >> $BASEDIR/restcomm_mem
    jcmd $RESTCOMM_PID GC.class_histogram  | grep org.mobicents.servlet.restcomm >> $BASEDIR/restcomm_mem

    jcmd $RESTCOMM_PID GC.run
    sleep 5

    echo "****************************************************************" >> $BASEDIR/restcomm_mem
    echo "GC Histogram after GC.run" >> $BASEDIR/restcomm_mem
    echo "****************************************************************" >> $BASEDIR/restcomm_mem
    jcmd $RESTCOMM_PID GC.class_histogram  | grep org.mobicents.servlet.restcomm >> $BASEDIR/restcomm_mem

    echo "****************************************************************" >> $BASEDIR/restcomm_mem
    echo "JVMTop" >> $BASEDIR/restcomm_mem
    echo "****************************************************************" >> $BASEDIR/restcomm_mem
    ./jvmtop.sh -n 1 >> $BASEDIR/restcomm_mem
